...
let grobify_term =
  ...
  and pow_tm = [%q {|(pow):complex->num->complex|} ] in
...
let holify_polynomial =
  let complex_ty = [%q {|:complex|} ]
  and pow_tm = [%q {|(pow):complex->num->complex|} ]
  and mk_mul = mk_binop [%q {|(*):complex->complex->complex|} ]
  and mk_add = mk_binop [%q {|(+):complex->complex->complex|} ]
  and zero_tm = [%q {|Cx(&0)|} ]
  and add_tm = [%q {|(+):complex->complex->complex|} ]
  and mul_tm = [%q {|(*):complex->complex->complex|} ]
  and complex_term_of_rat = curry mk_comb [%q {|Cx|} ] -| term_of_rat in
...
let vGROBNER_REFUTE =
  let add_tm = [%q {|(+):complex->complex->complex|} ]
  and mul_tm = [%q {|(*):complex->complex->complex|} ] in
  let vAPPLY_pth = vMATCH_MP(vSIMPLE_COMPLEX_ARITH
    [%q {|(x = y) ==> (x + Cx(--(&1)) * (y + Cx(&1)) = Cx(&0)) ==> F|} ])
  and vMK_ADD th1 th2 = vMK_COMB(vAP_TERM add_tm th1,th2) in
  let rec holify_lincombs vars cjs prfs =
    match prfs with
      [] -> cjs
    | (p::ps) ->
        if p = [] then holify_lincombs vars (cjs @ [vTRUTH]) ps else
        let holis = map (fun (n,q) -> n,holify_polynomial vars q) p in
        let ths =
          map (fun (n,m) -> vAP_TERM (mk_comb(mul_tm,m)) (el n cjs)) holis in
        let th = vCONV_RULE(vBINOP_CONV vCOMPLEX_POLY_CONV)
                          (end_itlist vMK_ADD ths) in
  ...
  fun tm ->
    ...
    let deps = map (dependencies -| snd) prfs
    ...
    vCONV_RULE vCOMPLEX_RAT_EQ_CONV th;;
...
let vCOMPLEX_ARITH =
  let pth0 = vSIMPLE_COMPLEX_ARITH [%q {|(x = y) <=> (x - y = Cx(&0))|} ]
  and pth1 = prove
   ([%q {|!x. ~(x = Cx(&0)) <=> ?z. z * x + Cx(&1) = Cx(&0)|} ],
    vCONV_TAC(vFULL_COMPLEX_QUELIM_CONV))
  and pth2a = prove
   ([%q {|!x y u v. ~(x = y) \/ ~(u = v) <=>
                ?w z. w * (x - y) + z * (u - v) + Cx(&1) = Cx(&0)|} ],
    vCONV_TAC(vFULL_COMPLEX_QUELIM_CONV))
  and pth2b = prove
   ([%q {|!x y. ~(x = y) <=> ?z. z * (x - y) + Cx(&1) = Cx(&0)|} ],
    vCONV_TAC(vFULL_COMPLEX_QUELIM_CONV))
  and pth3 = vTAUT [%q {|(p ==> F) ==> (~q <=> p) ==> q|} ] in
  let vGEN_PRENEX_CONV =
  ...
  let vINITIAL_CONV =
    vNNF_CONV +--->
    vGEN_REWRITE_CONV vONCE_DEPTH_CONV [pth1] +--->
    vGEN_REWRITE_CONV vONCE_DEPTH_CONV [pth2a] +--->
    vGEN_REWRITE_CONV vONCE_DEPTH_CONV [pth2b] +--->
    vONCE_DEPTH_CONV(vGEN_REWRITE_CONV vI [pth0] -|
                    check ((<>) [%q {|Cx(&0)|} ] -| rand)) +--->
    vGEN_PRENEX_CONV +--->
    vDNF_CONV in
  fun tm ->
    ...
    let th2 = end_itlist vSIMPLE_DISJ_CASES(map vGROBNER_REFUTE djs) in
    let th3 = itlist vSIMPLE_CHOOSE evs th2 in
    vSPECL avs (vMATCH_MP (vMATCH_MP pth3 (vDISCH_ALL th3)) th1);;
