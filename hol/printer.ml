...
let pp_print_type,pp_print_qtype =
  ...
  let rec sot pr ty =
    ...
    try Num.string_of_num(dest_finty ty) with Failure _ ->
...
let pp_print_term =
  ...
  let reverse_interface (s0,ty0) =
    ...
    try fst(find (fun (_s,(s',ty)) -> s' = s0 && can (type_match ty ty0) [])
                 (!the_interface))
    with Failure _ -> s0 in
  let vDEST_BINARY c tm =
      ...
      if s = "COND" && length args = 3 then
        ...
        (if prec = 1000 then pp_print_string fmt "(" else ();
         ...
         (if isalnum s ||
           s = "--" &&
           length args = 1 &&
           (try let l,_r = dest_comb(hd args) in
        ...
        let bargs =
          if vARIGHT s then
            let tms,tmt = splitlist (vDEST_BINARY hop) tm in tms@[tmt]
          else
            let tmt,tms = rev_splitlist (vDEST_BINARY hop) tm in tmt::tms in
