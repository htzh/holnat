...
let hashek_def = new_definition (parse_term "hashek = T")
let hashek_tm = (parse_term "hashek")
let hashek_thm = prove ((parse_term "hashek"), vSIMP_TAC[hashek_def])
let hashek_prop =
  prove((parse_term "((x /\\ hashek) ==> (y /\\ hashek)) ==> (x ==> y)"), vSIMP_TAC[hashek_def])
let hashek_eq = prove ((parse_term "~ hashek <=> F"), vSIMP_TAC[hashek_def])

let vMARK_CONCL label =
  vUSE_THEN label (fun th -> vUNDISCH_TAC (concl th)) ---->

  (fun g ->
     assert (is_imp (snd g));
     (* the first part of the implication *may* be a negation,
        but does not necessarily have to be *)
     assert (snd (dest_imp (snd g)) = (parse_term "F"));
     vALL_TAC g) ---->

  vREWRITE_TAC [vTAUT (parse_term "!t. (~t ==> F <=> t)")] ---->
  vPOP_ASSUM_LIST (fun l -> vASSUME_TAC (if l = [] then vTRUTH else end_itlist vCONJ l)) ---->

  (fun g -> assert (length (fst g) = 1); vALL_TAC g) ---->

  vW(vMAP_EVERY(vUNDISCH_TAC -| concl -| snd) -| fst) ---->
  vMATCH_MP_TAC hashek_prop ---->
  vDISCH_THEN (fun th -> vEVERY (map vASSUME_TAC (vCONJUNCTS th))) ---->
  vREFUTE_THEN vASSUME_TAC

let vMARK_ASSUMS = vRULE_ASSUM_TAC (vDISCH (parse_term "hashek"))
...
let concat_map (f : 'a -> 'b strom) (l : 'a strom) =
  let last = ref nil in
  let rec concat_map () =
...
and lit_ground (_p, a) = List.for_all term_ground a
...
let rec istriv sub x = function
  ...
  | Fnapp (_f, a) -> List.exists (istriv sub x) a && raise Unify;;
...
let rec ground_tm sub = function
  ...
  | Fnapp (_f,args) -> List.for_all (ground_tm sub) args
let ground_lit sub (_p, l) = List.for_all (ground_tm sub) l
...
let restore_subst ((suba, env), subl) =
...
let rec istriv suba x = function
  ...
  | Fnapp (_f, a) -> List.exists (istriv suba x) a && raise Unify

let add_subst ((suba, env), _subl as sub) x tm =
  if istriv suba x tm then sub
  else (suba.(x) <- Some tm; env := x :: !env; ((suba, env), !env))

let rec unify_tm ((suba, _env), _subl as sub) tm1 tm2 = match tm1,tm2 with
  | Fnapp (f,fargs), Fnapp (g,gargs) -> if f <> g then raise Unify
...
(* Unification with renaming of the second argument *)
let rec unify_tm_off off ((suba, _env), _subl as sub) t1 t2 = match t1,t2 with
  | Fnapp (f,fargs), Fnapp (g,gargs) -> if f <> g then raise Unify else
...
let to_list ((suba, _env), subl) = List.map
...
let rec ground_tm suba = function
  ...
  | Fnapp (_f, a) -> List.for_all (ground_tm suba) a

let ground_lit ((suba, _), _ as sub) (_p, l) =
...
let eq (sub, _off) l1 l2 = eq_lit sub l1 l2
...
let vPREPARE_COP_TAC ths =
  (* mostly from Meson *)
  vREFUTE_THEN (vLABEL_TAC "copconcl") ---->
  Meson.vPOLY_ASSUME_TAC (map vGEN_ALL ths) ---->

  (* CoP-specific part *)
  (Hashek.vMARK_CONCL "copconcl" |---> Hashek.vMARK_ASSUMS) ---->
...
let rec prove_lit opt sub (path, lem, lim) lit =
  ...
  let lemmas =
    if opt.improved_lemmas then begin
      ...
      Strom.map (fun _l -> if !copverb then Format.printf "lemma\n%!"; (sub, lit :: lem, Lemma))
...
let rec hashek_of_proof sub lem lit =
  ...
  | Extension (contra, prfs) ->
      ...
      let hth = if ths = [] then cth else vMATCH_MP cth (end_itlist vCONJ ths) in
      let ith = vPART_MATCH vI hth liti in
      Meson.finish_RULE (vDISCH (hol_negate (concl ith)) ith)
...
let vLEANCOP_DEEPEN opt ths =
  ...
  vEQ_MP Hashek.hashek_eq th

let vPREPARE_LEAN_TAC ths =
  Prepare.vPREPARE_COP_TAC ths ---->
...
let vPURE_LEANCOP opt gl =
  (vFIRST_ASSUM vCONTR_TAC |---> vW(vACCEPT_TAC -| vLEANCOP_DEEPEN opt -| map snd -| fst)) gl

let vGEN_LEANCOP_TAC opt ths = vPREPARE_LEAN_TAC ths ----> vPURE_LEANCOP opt
let vLEANCOP_TAC ths = vGEN_LEANCOP_TAC Cop.search_opts ths
let vLEANCOP ths tm = prove(tm, vLEANCOP_TAC ths)
...
let pp_print_matrix_ni f l = pp_print_matrix pp_print_null f l
...
let nth_clause i ((((j, _k), _v), _) as c) = if i = j then Some c else None
let nth_matrix i = function
    Lit _ -> None
  | Mat ((j, _k), _ as m) -> if i = j then Some m else None
    ...
    |> Strom.concat_map (fun ((_, args, ground, vars) as contra) ->
      ...
      if lim < 0 && not ground then nil
        ...
        Some (sub1, clext) ->
          ...
          |> Strom.concat_map (fun (sub2, position, ((i, _v), claB1), mi1) ->
...
(* Given a theorem `|- p1 \/ ... \/ pn` as well as theorems
   `p1 |- c`, ..., `pn |- c`, return `|- c`.
*)
let rec vDISJ_CASES_LIST th prfs =
  ...
  | [p] -> vMP (vDISCH (concl th) p) th
  | [p1; p2] -> vDISJ_CASES th p1 p2
  | p :: ps -> vDISJ_CASES th p (vDISJ_CASES_LIST (vASSUME (snd (dest_disj (concl th)))) ps);;

assert (vDISJ_CASES_LIST (vASSUME (parse_term "a \\/ b \\/ c"))
  [ vTAUT (parse_term "a ==> c \\/ b \\/ a") |> vUNDISCH
  ; vTAUT (parse_term "b ==> c \\/ b \\/ a") |> vUNDISCH
  ; vTAUT (parse_term "c ==> c \\/ b \\/ a") |> vUNDISCH] |> concl = (parse_term "c \\/ b \\/ a"));;


let vCONTRADICTION x y =
  let xc, yc = concl x, concl y in
  if !copverb then Format.printf "contra: %s with %s\n%!" (string_of_term xc) (string_of_term yc);
  if is_neg xc && not (is_neg yc) then vMP (vNOT_ELIM x) y
  else if is_neg yc && not (is_neg xc) then vMP (vNOT_ELIM y) x
  else failwith "no contradiction found"

let () = assert (concl (vCONTRADICTION (vASSUME (parse_term "~p")) (vASSUME (parse_term "p:bool"))) = (parse_term "F"));;
let () = assert (concl (vCONTRADICTION (vASSUME (parse_term "p:bool")) (vASSUME (parse_term "~p"))) = (parse_term "F"));;
...
let nth_disj j th =
  ...
  concl th |> disjuncts |> List.map vASSUME |> List.nth_rest j

let nth_conj i th =
  if !copverb then Format.printf "nth_conj %d %s\n%!" i (string_of_term (concl th));
  List.nth (vCONJUNCTS th) i
...
let spec_vars sub vs th =
  ...
  List.fold_left2 (fun th ty v -> vSPEC (inst_var sub ty v) th)
    th tys vs
...
and matrix_index_mapping mat = mat |>
  List.mapi (fun idx ((i, _v), cl) -> (i, idx) :: clause_index_mapping cl) |>
  List.concat
...
and bot_of_litmat_ext data acc thi = function
    Litext ((claL, _claR), _claC) ->
      ...
      let botj = vCONTRADICTION thj (concl thj |> flip_neg |> vASSUME) in
      acc2, vDISJ_CASES_LIST thi (botsL @ botj :: botsR)
  | Matext (j, (claL, _claR), clext) ->
      ...
      acc3, vDISJ_CASES_LIST thi (botsL @ botj :: botsR)

and bot_of_proof data lem th = function
    Mat (_j, mat), Decomposition (cla, ((i, _k), v), prfs) ->
      let i' = List.assoc i data.mapping in
      ...
      List.map vASSUME djs |> eat_proofs data (lem, prfs)
      |> snd |> vDISJ_CASES_LIST thi
  | Lit lit, prf ->
      ...
      begin match prf with
        Reduction -> flip_neg liti |> vASSUME |> vCONTRADICTION th
...
let start mat opt lim =
  ...
  Strom.of_list mati |> Strom.filter_map (function
      (((i, _), _v) as idx, ([Lit lit] as cla)) when lit = hashek ->
...
let vNANOCOP_DEEPEN opt ths =
  ...
  let gen_eq = map vGEN_ALL -| Equality.create_eq_axs opt.add_eq_symmetry -| map concl in
  let ths' =
  ...
  let th = end_itlist vCONJ ths' in
  ...
  assert (concl bot = (parse_term "F"));
  bot


(* Debugging tactic. *)
let vPRINT_TAC g = if !copverb then print_goal g; vALL_TAC g
let vPRINT_ID_TAC s g = if !copverb then print_endline s; vPRINT_TAC g

let vPREPARE_NANOCOP_TAC ths =
    Prepare.vPREPARE_COP_TAC ths ---->
    vRULE_ASSUM_TAC(vCONV_RULE(vDEPTH_CONV(vCHANGED_CONV(vASSOC_CONV vDISJ_ASSOC)))) ---->
    vRULE_ASSUM_TAC(vCONV_RULE(vDEPTH_CONV(vCHANGED_CONV(vASSOC_CONV vCONJ_ASSOC)))) ---->
    vREPEAT (vFIRST_X_ASSUM vSUBST_VAR_TAC)

let vPURE_NANOCOP opt gl =
  (vFIRST_ASSUM vCONTR_TAC |---> vW(vACCEPT_TAC -| vNANOCOP_DEEPEN opt -| map snd -| fst)) gl

let vGEN_NANOCOP_TAC opt ths = vPREPARE_NANOCOP_TAC ths ----> vPURE_NANOCOP opt

let vNANOCOP_TAC ths = vGEN_NANOCOP_TAC Cop.search_opts ths
let vNANOCOP ths tm = prove(tm, vNANOCOP_TAC ths)
...
let vLEANCOP_TAC = Leancop.vLEANCOP_TAC;;

let vLEANCOP = Leancop.vLEANCOP;;

let vNANOCOP_TAC = Nanocop.vNANOCOP_TAC;;

let vNANOCOP = Nanocop.vNANOCOP;;
