...
let vEXTENSION = try Cache.lookup_thm "EXTENSION" with _ ->  prove
...
let vIN_ELIM_THM = try Cache.lookup_thm "IN_ELIM_THM" with _ ->  prove
...
let vINSERT = try Cache.lookup_thm "INSERT" with _ ->  prove
 ((parse_term "x INSERT s = {y:A | y IN s \\/ (y = x)}"),
...
let vCHOICE_DEF = try Cache.lookup_thm "CHOICE_DEF" with _ ->  prove
...
let vNOT_EQUAL_SETS = try Cache.lookup_thm "NOT_EQUAL_SETS" with _ ->  prove
 ((parse_term "!s:A->bool. !t. ~(s = t) <=> ?x. x IN t <=> ~(x IN s)"),
  vSET_TAC[]);;

let vINSERT_RESTRICT = try Cache.lookup_thm "INSERT_RESTRICT" with _ ->  prove
...
let vMEMBER_NOT_EMPTY = try Cache.lookup_thm "MEMBER_NOT_EMPTY" with _ ->  prove
...
let vUNIV_NOT_EMPTY = try Cache.lookup_thm "UNIV_NOT_EMPTY" with _ ->  prove
 ((parse_term "~(UNIV:A->bool = EMPTY)"),
  vSET_TAC[]);;

let vEMPTY_NOT_UNIV = try Cache.lookup_thm "EMPTY_NOT_UNIV" with _ ->  prove
 ((parse_term "~(EMPTY:A->bool = UNIV)"),
  vSET_TAC[]);;

let vEQ_UNIV = try Cache.lookup_thm "EQ_UNIV" with _ ->  prove
...
let vSUBSET_TRANS = try Cache.lookup_thm "SUBSET_TRANS" with _ ->  prove
 ((parse_term "!(s:A->bool) t u. s SUBSET t /\\ t SUBSET u ==> s SUBSET u"),
  vSET_TAC[]);;

let vSUBSET_REFL = try Cache.lookup_thm "SUBSET_REFL" with _ ->  prove
 ((parse_term "!s:A->bool. s SUBSET s"),
  vSET_TAC[]);;

let vSUBSET_ANTISYM = try Cache.lookup_thm "SUBSET_ANTISYM" with _ ->  prove
 ((parse_term "!(s:A->bool) t. s SUBSET t /\\ t SUBSET s ==> s = t"),
  vSET_TAC[]);;

let vSUBSET_ANTISYM_EQ = try Cache.lookup_thm "SUBSET_ANTISYM_EQ" with _ ->  prove
 ((parse_term "!(s:A->bool) t. s SUBSET t /\\ t SUBSET s <=> s = t"),
  vSET_TAC[]);;

let vEMPTY_SUBSET = try Cache.lookup_thm "EMPTY_SUBSET" with _ ->  prove
 ((parse_term "!s:A->bool. EMPTY SUBSET s"),
  vSET_TAC[]);;

let vSUBSET_EMPTY = try Cache.lookup_thm "SUBSET_EMPTY" with _ ->  prove
 ((parse_term "!s:A->bool. s SUBSET EMPTY <=> (s = EMPTY)"),
  vSET_TAC[]);;

let vSUBSET_UNIV = try Cache.lookup_thm "SUBSET_UNIV" with _ ->  prove
 ((parse_term "!s:A->bool. s SUBSET UNIV"),
  vSET_TAC[]);;

let vUNIV_SUBSET = try Cache.lookup_thm "UNIV_SUBSET" with _ ->  prove
 ((parse_term "!s:A->bool. UNIV SUBSET s <=> (s = UNIV)"),
  vSET_TAC[]);;

let vSING_SUBSET = try Cache.lookup_thm "SING_SUBSET" with _ ->  prove
 ((parse_term "!s x. {x} SUBSET s <=> x IN s"),
  vSET_TAC[]);;

let vSUBSET_RESTRICT = try Cache.lookup_thm "SUBSET_RESTRICT" with _ ->  prove
...
let vPSUBSET_TRANS = try Cache.lookup_thm "PSUBSET_TRANS" with _ ->  prove
 ((parse_term "!(s:A->bool) t u. s PSUBSET t /\\ t PSUBSET u ==> s PSUBSET u"),
  vSET_TAC[]);;

let vPSUBSET_SUBSET_TRANS = try Cache.lookup_thm "PSUBSET_SUBSET_TRANS" with _ ->  prove
 ((parse_term "!(s:A->bool) t u. s PSUBSET t /\\ t SUBSET u ==> s PSUBSET u"),
  vSET_TAC[]);;

let vSUBSET_PSUBSET_TRANS = try Cache.lookup_thm "SUBSET_PSUBSET_TRANS" with _ ->  prove
 ((parse_term "!(s:A->bool) t u. s SUBSET t /\\ t PSUBSET u ==> s PSUBSET u"),
  vSET_TAC[]);;

let vPSUBSET_IRREFL = try Cache.lookup_thm "PSUBSET_IRREFL" with _ ->  prove
 ((parse_term "!s:A->bool. ~(s PSUBSET s)"),
  vSET_TAC[]);;

let vNOT_PSUBSET_EMPTY = try Cache.lookup_thm "NOT_PSUBSET_EMPTY" with _ ->  prove
 ((parse_term "!s:A->bool. ~(s PSUBSET EMPTY)"),
  vSET_TAC[]);;

let vNOT_UNIV_PSUBSET = try Cache.lookup_thm "NOT_UNIV_PSUBSET" with _ ->  prove
 ((parse_term "!s:A->bool. ~(UNIV PSUBSET s)"),
  vSET_TAC[]);;

let vPSUBSET_UNIV = try Cache.lookup_thm "PSUBSET_UNIV" with _ ->  prove
 ((parse_term "!s:A->bool. s PSUBSET UNIV <=> ?x. ~(x IN s)"),
  vSET_TAC[]);;

let vPSUBSET_ALT = try Cache.lookup_thm "PSUBSET_ALT" with _ ->  prove
...
let vUNION_ASSOC = try Cache.lookup_thm "UNION_ASSOC" with _ ->  prove
 ((parse_term "!(s:A->bool) t u. (s UNION t) UNION u = s UNION (t UNION u)"),
  vSET_TAC[]);;

let vUNION_IDEMPOT = try Cache.lookup_thm "UNION_IDEMPOT" with _ ->  prove
 ((parse_term "!s:A->bool. s UNION s = s"),
  vSET_TAC[]);;

let vUNION_COMM = try Cache.lookup_thm "UNION_COMM" with _ ->  prove
 ((parse_term "!(s:A->bool) t. s UNION t = t UNION s"),
  vSET_TAC[]);;

let vSUBSET_UNION = try Cache.lookup_thm "SUBSET_UNION" with _ ->  prove
 ((parse_term "(!s:A->bool. !t. s SUBSET (s UNION t)) /\\\n   (!s:A->bool. !t. s SUBSET (t UNION s))"),
  vSET_TAC[]);;

let vSUBSET_UNION_ABSORPTION = try Cache.lookup_thm "SUBSET_UNION_ABSORPTION" with _ ->  prove
 ((parse_term "!s:A->bool. !t. s SUBSET t <=> (s UNION t = t)"),
  vSET_TAC[]);;

let vUNION_EMPTY = try Cache.lookup_thm "UNION_EMPTY" with _ ->  prove
 ((parse_term "(!s:A->bool. EMPTY UNION s = s) /\\\n   (!s:A->bool. s UNION EMPTY = s)"),
  vSET_TAC[]);;

let vUNION_UNIV = try Cache.lookup_thm "UNION_UNIV" with _ ->  prove
 ((parse_term "(!s:A->bool. UNIV UNION s = UNIV) /\\\n   (!s:A->bool. s UNION UNIV = UNIV)"),
  vSET_TAC[]);;

let vEMPTY_UNION = try Cache.lookup_thm "EMPTY_UNION" with _ ->  prove
 ((parse_term "!s:A->bool. !t. (s UNION t = EMPTY) <=> (s = EMPTY) /\\ (t = EMPTY)"),
  vSET_TAC[]);;

let vUNION_SUBSET = try Cache.lookup_thm "UNION_SUBSET" with _ ->  prove
 ((parse_term "!s t u. (s UNION t) SUBSET u <=> s SUBSET u /\\ t SUBSET u"),
  vSET_TAC[]);;

let vUNION_RESTRICT = try Cache.lookup_thm "UNION_RESTRICT" with _ ->  prove
 ((parse_term "!P s t.\n        {x | x IN (s UNION t) /\\ P x} =\n        {x | x IN s /\\ P x} UNION {x | x IN t /\\ P x}"),
  vSET_TAC[]);;

let vFORALL_SUBSET_UNION = try Cache.lookup_thm "FORALL_SUBSET_UNION" with _ ->  prove
...
let vEXISTS_SUBSET_UNION = try Cache.lookup_thm "EXISTS_SUBSET_UNION" with _ ->  prove
 ((parse_term "!t u:A->bool.\n        (?s. s SUBSET t UNION u /\\ P s) <=>\n        (?t' u'. t' SUBSET t /\\ u' SUBSET u /\\ P(t' UNION u'))"),
  vREWRITE_TAC[vMESON[] (parse_term "(?x. P x /\\ Q x) <=> ~(!x. P x ==> ~Q x)")] ---->
  vREWRITE_TAC[vFORALL_SUBSET_UNION] ----> vMESON_TAC[]);;

let vFORALL_SUBSET_INSERT = try Cache.lookup_thm "FORALL_SUBSET_INSERT" with _ ->  prove
...
let vEXISTS_SUBSET_INSERT = try Cache.lookup_thm "EXISTS_SUBSET_INSERT" with _ ->  prove
 ((parse_term "!a:A t. (?s. s SUBSET a INSERT t /\\ P s) <=>\n           (?s. s SUBSET t /\\ (P s \\/ P (a INSERT s)))"),
  vREWRITE_TAC[vMESON[] (parse_term "(?x. P x /\\ Q x) <=> ~(!x. P x ==> ~Q x)")] ---->
...
let vINTER_ASSOC = try Cache.lookup_thm "INTER_ASSOC" with _ ->  prove
 ((parse_term "!(s:A->bool) t u. (s INTER t) INTER u = s INTER (t INTER u)"),
  vSET_TAC[]);;

let vINTER_IDEMPOT = try Cache.lookup_thm "INTER_IDEMPOT" with _ ->  prove
 ((parse_term "!s:A->bool. s INTER s = s"),
  vSET_TAC[]);;

let vINTER_COMM = try Cache.lookup_thm "INTER_COMM" with _ ->  prove
 ((parse_term "!(s:A->bool) t. s INTER t = t INTER s"),
  vSET_TAC[]);;

let vINTER_SUBSET = try Cache.lookup_thm "INTER_SUBSET" with _ ->  prove
 ((parse_term "(!s:A->bool. !t. (s INTER t) SUBSET s) /\\\n   (!s:A->bool. !t. (t INTER s) SUBSET s)"),
  vSET_TAC[]);;

let vSUBSET_INTER_ABSORPTION = try Cache.lookup_thm "SUBSET_INTER_ABSORPTION" with _ ->  prove
 ((parse_term "!s:A->bool. !t. s SUBSET t <=> (s INTER t = s)"),
  vSET_TAC[]);;

let vINTER_EMPTY = try Cache.lookup_thm "INTER_EMPTY" with _ ->  prove
 ((parse_term "(!s:A->bool. EMPTY INTER s = EMPTY) /\\\n   (!s:A->bool. s INTER EMPTY = EMPTY)"),
  vSET_TAC[]);;

let vINTER_UNIV = try Cache.lookup_thm "INTER_UNIV" with _ ->  prove
 ((parse_term "(!s:A->bool. UNIV INTER s = s) /\\\n   (!s:A->bool. s INTER UNIV = s)"),
  vSET_TAC[]);;

let vSUBSET_INTER = try Cache.lookup_thm "SUBSET_INTER" with _ ->  prove
 ((parse_term "!s t u. s SUBSET (t INTER u) <=> s SUBSET t /\\ s SUBSET u"),
  vSET_TAC[]);;

let vINTER_RESTRICT = try Cache.lookup_thm "INTER_RESTRICT" with _ ->  prove
...
let vUNION_OVER_INTER = try Cache.lookup_thm "UNION_OVER_INTER" with _ ->  prove
 ((parse_term "!s:A->bool. !t u. s INTER (t UNION u) = (s INTER t) UNION (s INTER u)"),
  vSET_TAC[]);;

let vINTER_OVER_UNION = try Cache.lookup_thm "INTER_OVER_UNION" with _ ->  prove
...
let vIN_DISJOINT = try Cache.lookup_thm "IN_DISJOINT" with _ ->  prove
 ((parse_term "!s:A->bool. !t. DISJOINT s t <=> ~(?x. x IN s /\\ x IN t)"),
  vSET_TAC[]);;

let vDISJOINT_SYM = try Cache.lookup_thm "DISJOINT_SYM" with _ ->  prove
 ((parse_term "!s:A->bool. !t. DISJOINT s t <=> DISJOINT t s"),
  vSET_TAC[]);;

let vDISJOINT_EMPTY = try Cache.lookup_thm "DISJOINT_EMPTY" with _ ->  prove
 ((parse_term "!s:A->bool. DISJOINT EMPTY s /\\ DISJOINT s EMPTY"),
  vSET_TAC[]);;

let vDISJOINT_EMPTY_REFL = try Cache.lookup_thm "DISJOINT_EMPTY_REFL" with _ ->  prove
 ((parse_term "!s:A->bool. (s = EMPTY) <=> (DISJOINT s s)"),
  vSET_TAC[]);;

let vDISJOINT_UNION = try Cache.lookup_thm "DISJOINT_UNION" with _ ->  prove
 ((parse_term "!s:A->bool. !t u. DISJOINT (s UNION t) u <=> DISJOINT s u /\\ DISJOINT t u"),
  vSET_TAC[]);;

let vDISJOINT_SING = try Cache.lookup_thm "DISJOINT_SING" with _ ->  prove
...
let vDIFF_EMPTY = try Cache.lookup_thm "DIFF_EMPTY" with _ ->  prove
 ((parse_term "!s:A->bool. s DIFF EMPTY = s"),
  vSET_TAC[]);;

let vEMPTY_DIFF = try Cache.lookup_thm "EMPTY_DIFF" with _ ->  prove
 ((parse_term "!s:A->bool. EMPTY DIFF s = EMPTY"),
  vSET_TAC[]);;

let vDIFF_UNIV = try Cache.lookup_thm "DIFF_UNIV" with _ ->  prove
 ((parse_term "!s:A->bool. s DIFF UNIV = EMPTY"),
  vSET_TAC[]);;

let vDIFF_DIFF = try Cache.lookup_thm "DIFF_DIFF" with _ ->  prove
 ((parse_term "!s:A->bool. !t. (s DIFF t) DIFF t = s DIFF t"),
  vSET_TAC[]);;

let vDIFF_EQ_EMPTY = try Cache.lookup_thm "DIFF_EQ_EMPTY" with _ ->  prove
 ((parse_term "!s:A->bool. s DIFF s = EMPTY"),
  vSET_TAC[]);;

let vSUBSET_DIFF = try Cache.lookup_thm "SUBSET_DIFF" with _ ->  prove
 ((parse_term "!s t. (s DIFF t) SUBSET s"),
  vSET_TAC[]);;

let vCOMPL_COMPL = try Cache.lookup_thm "COMPL_COMPL" with _ ->  prove
 ((parse_term "!s. (:A) DIFF ((:A) DIFF s) = s"),
  vSET_TAC[]);;

let vDIFF_RESTRICT = try Cache.lookup_thm "DIFF_RESTRICT" with _ ->  prove
...
let vCOMPONENT = try Cache.lookup_thm "COMPONENT" with _ ->  prove
 ((parse_term "!x:A. !s. x IN (x INSERT s)"),
  vSET_TAC[]);;

let vDECOMPOSITION = try Cache.lookup_thm "DECOMPOSITION" with _ ->  prove
...
let vSET_CASES = try Cache.lookup_thm "SET_CASES" with _ ->  prove
 ((parse_term "!s:A->bool. (s = EMPTY) \\/ ?x:A. ?t. (s = x INSERT t) /\\ ~(x IN t)"),
  vMESON_TAC[vMEMBER_NOT_EMPTY; vDECOMPOSITION]);;

let vABSORPTION = try Cache.lookup_thm "ABSORPTION" with _ ->  prove
 ((parse_term "!x:A. !s. x IN s <=> (x INSERT s = s)"),
  vSET_TAC[]);;

let vINSERT_INSERT = try Cache.lookup_thm "INSERT_INSERT" with _ ->  prove
 ((parse_term "!x:A. !s. x INSERT (x INSERT s) = x INSERT s"),
  vSET_TAC[]);;

let vINSERT_COMM = try Cache.lookup_thm "INSERT_COMM" with _ ->  prove
 ((parse_term "!x:A. !y s. x INSERT (y INSERT s) = y INSERT (x INSERT s)"),
  vSET_TAC[]);;

let vINSERT_UNIV = try Cache.lookup_thm "INSERT_UNIV" with _ ->  prove
 ((parse_term "!x:A. x INSERT UNIV = UNIV"),
  vSET_TAC[]);;

let vNOT_INSERT_EMPTY = try Cache.lookup_thm "NOT_INSERT_EMPTY" with _ ->  prove
 ((parse_term "!x:A. !s. ~(x INSERT s = EMPTY)"),
  vSET_TAC[]);;

let vNOT_EMPTY_INSERT = try Cache.lookup_thm "NOT_EMPTY_INSERT" with _ ->  prove
 ((parse_term "!x:A. !s. ~(EMPTY = x INSERT s)"),
  vSET_TAC[]);;

let vINSERT_UNION = try Cache.lookup_thm "INSERT_UNION" with _ ->  prove
 ((parse_term "!x:A. !s t. (x INSERT s) UNION t =\n               if x IN t then s UNION t else x INSERT (s UNION t)"),
  vREPEAT vGEN_TAC ----> vCOND_CASES_TAC ---->
  vPOP_ASSUM vMP_TAC ----> vSET_TAC[]);;

let vINSERT_UNION_EQ = try Cache.lookup_thm "INSERT_UNION_EQ" with _ ->  prove
 ((parse_term "!x:A. !s t. (x INSERT s) UNION t = x INSERT (s UNION t)"),
  vSET_TAC[]);;

let vINSERT_INTER = try Cache.lookup_thm "INSERT_INTER" with _ ->  prove
 ((parse_term "!x:A. !s t. (x INSERT s) INTER t =\n               if x IN t then x INSERT (s INTER t) else s INTER t"),
  vREPEAT vGEN_TAC ----> vCOND_CASES_TAC ---->
  vPOP_ASSUM vMP_TAC ----> vSET_TAC[]);;

let vDISJOINT_INSERT = try Cache.lookup_thm "DISJOINT_INSERT" with _ ->  prove
 ((parse_term "!(x:A) s t. DISJOINT (x INSERT s) t <=> (DISJOINT s t) /\\ ~(x IN t)"),
  vSET_TAC[]);;

let vINSERT_SUBSET = try Cache.lookup_thm "INSERT_SUBSET" with _ ->  prove
 ((parse_term "!x:A. !s t. (x INSERT s) SUBSET t <=> (x IN t /\\ s SUBSET t)"),
  vSET_TAC[]);;

let vSUBSET_INSERT = try Cache.lookup_thm "SUBSET_INSERT" with _ ->  prove
 ((parse_term "!x:A. !s. ~(x IN s) ==> !t. s SUBSET (x INSERT t) <=> s SUBSET t"),
  vSET_TAC[]);;

let vINSERT_DIFF = try Cache.lookup_thm "INSERT_DIFF" with _ ->  prove
 ((parse_term "!s t. !x:A. (x INSERT s) DIFF t =\n               if x IN t then s DIFF t else x INSERT (s DIFF t)"),
  vREPEAT vGEN_TAC ----> vCOND_CASES_TAC ---->
  vPOP_ASSUM vMP_TAC ----> vSET_TAC[]);;

let vINSERT_AC = try Cache.lookup_thm "INSERT_AC" with _ ->  prove
 ((parse_term "(x INSERT (y INSERT s) = y INSERT (x INSERT s)) /\\\n   (x INSERT (x INSERT s) = x INSERT s)"),
  vREWRITE_TAC[vINSERT_COMM; vINSERT_INSERT]);;

let vINTER_ACI = try Cache.lookup_thm "INTER_ACI" with _ ->  prove
...
let vUNION_ACI = try Cache.lookup_thm "UNION_ACI" with _ ->  prove
...
let vDELETE_NON_ELEMENT = try Cache.lookup_thm "DELETE_NON_ELEMENT" with _ ->  prove
 ((parse_term "!x:A. !s. ~(x IN s) <=> (s DELETE x = s)"),
  vSET_TAC[]);;

let vIN_DELETE_EQ = try Cache.lookup_thm "IN_DELETE_EQ" with _ ->  prove
 ((parse_term "!s x. !x':A.\n     (x IN s <=> x' IN s) <=> (x IN (s DELETE x') <=> x' IN (s DELETE x))"),
  vSET_TAC[]);;

let vEMPTY_DELETE = try Cache.lookup_thm "EMPTY_DELETE" with _ ->  prove
 ((parse_term "!x:A. EMPTY DELETE x = EMPTY"),
  vSET_TAC[]);;

let vDELETE_DELETE = try Cache.lookup_thm "DELETE_DELETE" with _ ->  prove
 ((parse_term "!x:A. !s. (s DELETE x) DELETE x = s DELETE x"),
  vSET_TAC[]);;

let vDELETE_COMM = try Cache.lookup_thm "DELETE_COMM" with _ ->  prove
 ((parse_term "!x:A. !y. !s. (s DELETE x) DELETE y = (s DELETE y) DELETE x"),
  vSET_TAC[]);;

let vDELETE_SUBSET = try Cache.lookup_thm "DELETE_SUBSET" with _ ->  prove
 ((parse_term "!x:A. !s. (s DELETE x) SUBSET s"),
  vSET_TAC[]);;

let vSUBSET_DELETE = try Cache.lookup_thm "SUBSET_DELETE" with _ ->  prove
 ((parse_term "!x:A. !s t. s SUBSET (t DELETE x) <=> ~(x IN s) /\\ (s SUBSET t)"),
  vSET_TAC[]);;

let vSUBSET_INSERT_DELETE = try Cache.lookup_thm "SUBSET_INSERT_DELETE" with _ ->  prove
 ((parse_term "!x:A. !s t. s SUBSET (x INSERT t) <=> ((s DELETE x) SUBSET t)"),
  vSET_TAC[]);;

let vDIFF_INSERT = try Cache.lookup_thm "DIFF_INSERT" with _ ->  prove
 ((parse_term "!s t. !x:A. s DIFF (x INSERT t) = (s DELETE x) DIFF t"),
  vSET_TAC[]);;

let vPSUBSET_INSERT_SUBSET = try Cache.lookup_thm "PSUBSET_INSERT_SUBSET" with _ ->  prove
 ((parse_term "!s t. s PSUBSET t <=> ?x:A. ~(x IN s) /\\ (x INSERT s) SUBSET t"),
  vSET_TAC[]);;

let vDELETE_INSERT = try Cache.lookup_thm "DELETE_INSERT" with _ ->  prove
 ((parse_term "!x:A. !y s.\n      (x INSERT s) DELETE y =\n        if x = y then s DELETE y else x INSERT (s DELETE y)"),
  vREPEAT vGEN_TAC ----> vCOND_CASES_TAC ---->
  vPOP_ASSUM vMP_TAC ----> vSET_TAC[]);;

let vINSERT_DELETE = try Cache.lookup_thm "INSERT_DELETE" with _ ->  prove
 ((parse_term "!x:A. !s. x IN s ==> (x INSERT (s DELETE x) = s)"),
  vSET_TAC[]);;

let vDELETE_INTER = try Cache.lookup_thm "DELETE_INTER" with _ ->  prove
 ((parse_term "!s t. !x:A. (s DELETE x) INTER t = (s INTER t) DELETE x"),
  vSET_TAC[]);;

let vDISJOINT_DELETE_SYM = try Cache.lookup_thm "DISJOINT_DELETE_SYM" with _ ->  prove
...
let vUNIONS_0 = try Cache.lookup_thm "UNIONS_0" with _ ->  prove
 ((parse_term "UNIONS {} = {}"),
  vSET_TAC[]);;

let vUNIONS_1 = try Cache.lookup_thm "UNIONS_1" with _ ->  prove
 ((parse_term "UNIONS {s} = s"),
  vSET_TAC[]);;

let vUNIONS_2 = try Cache.lookup_thm "UNIONS_2" with _ ->  prove
 ((parse_term "UNIONS {s,t} = s UNION t"),
  vSET_TAC[]);;

let vUNIONS_INSERT = try Cache.lookup_thm "UNIONS_INSERT" with _ ->  prove
 ((parse_term "UNIONS (s INSERT u) = s UNION (UNIONS u)"),
  vSET_TAC[]);;

let vFORALL_IN_UNIONS = try Cache.lookup_thm "FORALL_IN_UNIONS" with _ ->  prove
 ((parse_term "!P s. (!x. x IN UNIONS s ==> P x) <=> !t x. t IN s /\\ x IN t ==> P x"),
  vSET_TAC[]);;

let vEXISTS_IN_UNIONS = try Cache.lookup_thm "EXISTS_IN_UNIONS" with _ ->  prove
 ((parse_term "!P s. (?x. x IN UNIONS s /\\ P x) <=> (?t x. t IN s /\\ x IN t /\\ P x)"),
  vSET_TAC[]);;

let vEMPTY_UNIONS = try Cache.lookup_thm "EMPTY_UNIONS" with _ ->  prove
 ((parse_term "!s. (UNIONS s = {}) <=> !t. t IN s ==> t = {}"),
  vSET_TAC[]);;

let vINTER_UNIONS = try Cache.lookup_thm "INTER_UNIONS" with _ ->  prove
...
let vUNIONS_SUBSET = try Cache.lookup_thm "UNIONS_SUBSET" with _ ->  prove
 ((parse_term "!f t. UNIONS f SUBSET t <=> !s. s IN f ==> s SUBSET t"),
  vSET_TAC[]);;

let vSUBSET_UNIONS = try Cache.lookup_thm "SUBSET_UNIONS" with _ ->  prove
 ((parse_term "!f g. f SUBSET g ==> UNIONS f SUBSET UNIONS g"),
  vSET_TAC[]);;

let vUNIONS_UNION = try Cache.lookup_thm "UNIONS_UNION" with _ ->  prove
 ((parse_term "!s t. UNIONS(s UNION t) = (UNIONS s) UNION (UNIONS t)"),
  vSET_TAC[]);;

let vINTERS_UNION = try Cache.lookup_thm "INTERS_UNION" with _ ->  prove
 ((parse_term "!s t. INTERS (s UNION t) = INTERS s INTER INTERS t"),
  vSET_TAC[]);;

let vUNIONS_MONO = try Cache.lookup_thm "UNIONS_MONO" with _ ->  prove
 ((parse_term "(!x. x IN s ==> ?y. y IN t /\\ x SUBSET y) ==> UNIONS s SUBSET UNIONS t"),
  vSET_TAC[]);;

let vUNIONS_MONO_IMAGE = try Cache.lookup_thm "UNIONS_MONO_IMAGE" with _ ->  prove
 ((parse_term "(!x. x IN s ==> f x SUBSET g x)\n   ==> UNIONS(IMAGE f s) SUBSET UNIONS(IMAGE g s)"),
  vSET_TAC[]);;

let vUNIONS_UNIV = try Cache.lookup_thm "UNIONS_UNIV" with _ ->  prove
 ((parse_term "UNIONS (:A->bool) = (:A)"),
  vREWRITE_TAC[vEXTENSION; vIN_UNIONS; vIN_UNIV] ---->
  vMESON_TAC[vIN_SING]);;

let vUNIONS_INSERT_EMPTY = try Cache.lookup_thm "UNIONS_INSERT_EMPTY" with _ ->  prove
 ((parse_term "!s. UNIONS({} INSERT s) = UNIONS s"),
  vONCE_REWRITE_TAC[vEXTENSION] ---->
  vREWRITE_TAC[vIN_UNIONS; vIN_INSERT] ----> vMESON_TAC[vNOT_IN_EMPTY]);;

let vUNIONS_DELETE_EMPTY = try Cache.lookup_thm "UNIONS_DELETE_EMPTY" with _ ->  prove
...
let vINTERS_0 = try Cache.lookup_thm "INTERS_0" with _ ->  prove
 ((parse_term "INTERS {} = (:A)"),
  vSET_TAC[]);;

let vINTERS_1 = try Cache.lookup_thm "INTERS_1" with _ ->  prove
 ((parse_term "INTERS {s} = s"),
  vSET_TAC[]);;

let vINTERS_2 = try Cache.lookup_thm "INTERS_2" with _ ->  prove
 ((parse_term "INTERS {s,t} = s INTER t"),
  vSET_TAC[]);;

let vINTERS_INSERT = try Cache.lookup_thm "INTERS_INSERT" with _ ->  prove
 ((parse_term "INTERS (s INSERT u) = s INTER (INTERS u)"),
  vSET_TAC[]);;

let vSUBSET_INTERS = try Cache.lookup_thm "SUBSET_INTERS" with _ ->  prove
 ((parse_term "!s f. s SUBSET INTERS f <=> (!t. t IN f ==> s SUBSET t)"),
  vSET_TAC[]);;

let vINTERS_SUBSET = try Cache.lookup_thm "INTERS_SUBSET" with _ ->  prove
 ((parse_term "!u s:A->bool.\n    ~(u = {}) /\\ (!t. t IN u ==> t SUBSET s) ==> INTERS u SUBSET s"),
  vSET_TAC[]);;

let vINTERS_SUBSET_STRONG = try Cache.lookup_thm "INTERS_SUBSET_STRONG" with _ ->  prove
 ((parse_term "!u s:A->bool. (?t. t IN u /\\ t SUBSET s) ==> INTERS u SUBSET s"),
  vSET_TAC[]);;

let vINTERS_ANTIMONO = try Cache.lookup_thm "INTERS_ANTIMONO" with _ ->  prove
 ((parse_term "!f g. g SUBSET f ==> INTERS f SUBSET INTERS g"),
  vSET_TAC[]);;

let vINTERS_EQ_UNIV = try Cache.lookup_thm "INTERS_EQ_UNIV" with _ ->  prove
 ((parse_term "!f. INTERS f = (:A) <=> !s. s IN f ==> s = (:A)"),
  vSET_TAC[]);;

let vINTERS_ANTIMONO_GEN = try Cache.lookup_thm "INTERS_ANTIMONO_GEN" with _ ->  prove
...
let vIMAGE_DIFF_INJ = try Cache.lookup_thm "IMAGE_DIFF_INJ" with _ ->  prove
 ((parse_term "!f:A->B s t.\n        (!x y. x IN s /\\ y IN t /\\ f x = f y ==> x = y)\n        ==> IMAGE f (s DIFF t) = IMAGE f s DIFF IMAGE f t"),
  vSET_TAC[]);;

let vIMAGE_DIFF_INJ_ALT = try Cache.lookup_thm "IMAGE_DIFF_INJ_ALT" with _ ->  prove
 ((parse_term "!f:A->B s t.\n        (!x y. x IN s /\\ y IN s /\\ f x = f y ==> x = y) /\\ t SUBSET s\n        ==> IMAGE f (s DIFF t) = IMAGE f s DIFF IMAGE f t"),
  vSET_TAC[]);;

let vIMAGE_DELETE_INJ = try Cache.lookup_thm "IMAGE_DELETE_INJ" with _ ->  prove
 ((parse_term "!f:A->B s a.\n        (!x. x IN s /\\ f x = f a ==> x = a)\n        ==> IMAGE f (s DELETE a) = IMAGE f s DELETE f a"),
  vSET_TAC[]);;

let vIMAGE_DELETE_INJ_ALT = try Cache.lookup_thm "IMAGE_DELETE_INJ_ALT" with _ ->  prove
 ((parse_term "!f:A->B s a.\n        (!x y. x IN s /\\ y IN s /\\ f x = f y ==> x = y) /\\ a IN s\n        ==> IMAGE f (s DELETE a) = IMAGE f s DELETE f a"),
  vSET_TAC[]);;

let vIMAGE_EQ_EMPTY = try Cache.lookup_thm "IMAGE_EQ_EMPTY" with _ ->  prove
 ((parse_term "!f s. (IMAGE f s = {}) <=> (s = {})"),
  vREWRITE_TAC[vEXTENSION; vNOT_IN_EMPTY; vIN_IMAGE] ----> vMESON_TAC[]);;

let vFORALL_IN_IMAGE = try Cache.lookup_thm "FORALL_IN_IMAGE" with _ ->  prove
 ((parse_term "!f s. (!y. y IN IMAGE f s ==> P y) <=> (!x. x IN s ==> P(f x))"),
  vREWRITE_TAC[vIN_IMAGE] ----> vMESON_TAC[]);;

let vEXISTS_IN_IMAGE = try Cache.lookup_thm "EXISTS_IN_IMAGE" with _ ->  prove
 ((parse_term "!f s. (?y. y IN IMAGE f s /\\ P y) <=> ?x. x IN s /\\ P(f x)"),
  vREWRITE_TAC[vIN_IMAGE] ----> vMESON_TAC[]);;

let vFORALL_IN_IMAGE_2 = try Cache.lookup_thm "FORALL_IN_IMAGE_2" with _ ->  prove
 ((parse_term "!f P s. (!x y. x IN IMAGE f s /\\ y IN IMAGE f s ==> P x y) <=>\n           (!x y. x IN s /\\ y IN s ==> P (f x) (f y))"),
  vSET_TAC[]);;

let vIMAGE_CONST = try Cache.lookup_thm "IMAGE_CONST" with _ ->  prove
...
let vSIMPLE_IMAGE = try Cache.lookup_thm "SIMPLE_IMAGE" with _ ->  prove
 ((parse_term "!f s. {f x | x IN s} = IMAGE f s"),
  vREWRITE_TAC[vEXTENSION; vIN_ELIM_THM; vIN_IMAGE] ----> vMESON_TAC[]);;

let vSIMPLE_IMAGE_GEN = try Cache.lookup_thm "SIMPLE_IMAGE_GEN" with _ ->  prove
 ((parse_term "!f P. {f x | P x} = IMAGE f {x | P x}"),
  vSET_TAC[]);;

let vIMAGE_UNIONS = try Cache.lookup_thm "IMAGE_UNIONS" with _ ->  prove
...
let vFUN_IN_IMAGE = try Cache.lookup_thm "FUN_IN_IMAGE" with _ ->  prove
 ((parse_term "!f s x. x IN s ==> f(x) IN IMAGE f s"),
  vSET_TAC[]);;

let vSURJECTIVE_IMAGE_EQ = try Cache.lookup_thm "SURJECTIVE_IMAGE_EQ" with _ ->  prove
...
let vEMPTY_GSPEC = try Cache.lookup_thm "EMPTY_GSPEC" with _ ->  prove
 ((parse_term "{x | F} = {}"),
  vSET_TAC[]);;

let vUNIV_GSPEC = try Cache.lookup_thm "UNIV_GSPEC" with _ ->  prove
 ((parse_term "{x | T} = UNIV"),
  vSET_TAC[]);;

let vSING_GSPEC = try Cache.lookup_thm "SING_GSPEC" with _ ->  prove
 ((parse_term "(!a. {x | x = a} = {a}) /\\\n   (!a. {x | a = x} = {a})"),
  vSET_TAC[]);;

let vIN_GSPEC = try Cache.lookup_thm "IN_GSPEC" with _ ->  prove
 ((parse_term "!s:A->bool. {x | x IN s} = s"),
  vSET_TAC[]);;

let vIN_ELIM_PAIR_THM = try Cache.lookup_thm "IN_ELIM_PAIR_THM" with _ ->  prove
 ((parse_term "!P a b. (a,b) IN {(x,y) | P x y} <=> P a b"),
  vREWRITE_TAC[vIN_ELIM_THM] ----> vMESON_TAC[vPAIR_EQ]);;

let vIN_ELIM_TRIPLE_THM = try Cache.lookup_thm "IN_ELIM_TRIPLE_THM" with _ ->  prove
...
let vIN_ELIM_QUAD_THM = try Cache.lookup_thm "IN_ELIM_QUAD_THM" with _ ->  prove
 ((parse_term "(!P a b c d. (a,b,c,d) IN {w,x,y,z | P w x y z} <=> P a b c d) /\\\n   (!P a b c d. ((a,b),(c,d)) IN {(w,x),(y,z) | P w x y z} <=> P a b c d) /\\\n   (!P a b c d. (((a,b),c),d) IN {(((w,x),y),z) | P w x y z} <=> P a b c d)"),
  vREWRITE_TAC[vIN_ELIM_THM; vPAIR_EQ] ----> vMESON_TAC[]);;

let vSET_PAIR_THM = try Cache.lookup_thm "SET_PAIR_THM" with _ ->  prove
 ((parse_term "!P. {p | P p} = {(a,b) | P(a,b)}"),
  vREWRITE_TAC[vEXTENSION; vFORALL_PAIR_THM; vIN_ELIM_THM; vIN_ELIM_PAIR_THM]);;

let vFORALL_IN_GSPEC = try Cache.lookup_thm "FORALL_IN_GSPEC" with _ ->  prove
...
let vEXISTS_IN_GSPEC = try Cache.lookup_thm "EXISTS_IN_GSPEC" with _ ->  prove
 ((parse_term "(!P f. (?z. z IN {f x | P x} /\\ Q z) <=> (?x. P x /\\ Q(f x))) /\\\n   (!P f. (?z. z IN {f x y | P x y} /\\ Q z) <=>\n          (?x y. P x y /\\ Q(f x y))) /\\\n   (!P f. (?z. z IN {f w x y | P w x y} /\\ Q z) <=>\n          (?w x y. P w x y /\\ Q(f w x y))) /\\\n   (!P f. (?z. z IN {f v w x y | P v w x y} /\\ Q z) <=>\n          (?v w x y. P v w x y /\\ Q(f v w x y)))"),
  vSET_TAC[]);;

let vSET_PROVE_CASES = try Cache.lookup_thm "SET_PROVE_CASES" with _ ->  prove
 ((parse_term "!P:(A->bool)->bool.\n       P {} /\\ (!a s. ~(a IN s) ==> P(a INSERT s))\n       ==> !s. P s"),
  vMESON_TAC[vSET_CASES]);;

let vUNIONS_IMAGE = try Cache.lookup_thm "UNIONS_IMAGE" with _ ->  prove
...
let vINTERS_IMAGE = try Cache.lookup_thm "INTERS_IMAGE" with _ ->  prove
 ((parse_term "!f s. INTERS (IMAGE f s) = {y | !x. x IN s ==> y IN f x}"),
  vREPEAT vGEN_TAC ---->  vGEN_REWRITE_TAC vI [vEXTENSION] ---->
  vREWRITE_TAC[vIN_INTERS; vIN_IMAGE; vIN_ELIM_THM] ----> vMESON_TAC[]);;

let vUNIONS_GSPEC = try Cache.lookup_thm "UNIONS_GSPEC" with _ ->  prove
...
let vINTERS_GSPEC = try Cache.lookup_thm "INTERS_GSPEC" with _ ->  prove
...
let vUNIONS_SINGS_GEN = try Cache.lookup_thm "UNIONS_SINGS_GEN" with _ ->  prove
 ((parse_term "!P:A->bool. UNIONS {{x} | P x} = {x | P x}"),
  vREWRITE_TAC[vUNIONS_GSPEC] ----> vSET_TAC[]);;

let vUNIONS_SINGS = try Cache.lookup_thm "UNIONS_SINGS" with _ ->  prove
 ((parse_term "!s:A->bool. UNIONS {{x} | x IN s} = s"),
  vREWRITE_TAC[vUNIONS_GSPEC] ----> vSET_TAC[]);;

let vIMAGE_INTERS = try Cache.lookup_thm "IMAGE_INTERS" with _ ->  prove
 ((parse_term "!f s. ~(s = {}) /\\\n         (!x y. x IN UNIONS s /\\ y IN UNIONS s /\\ f x = f y ==> x = y)\n         ==> IMAGE f (INTERS s) = INTERS(IMAGE (IMAGE f) s)"),
  vREWRITE_TAC[vINTERS_IMAGE] ----> vSET_TAC[]);;

let vDIFF_INTERS = try Cache.lookup_thm "DIFF_INTERS" with _ ->  prove
 ((parse_term "!u s. u DIFF INTERS s = UNIONS {u DIFF t | t IN s}"),
  vREWRITE_TAC[vUNIONS_GSPEC] ----> vSET_TAC[]);;

let vINTERS_UNIONS = try Cache.lookup_thm "INTERS_UNIONS" with _ ->  prove
 ((parse_term "!s. INTERS s = UNIV DIFF (UNIONS {UNIV DIFF t | t IN s})"),
  vREWRITE_TAC[vGSYM vDIFF_INTERS] ----> vSET_TAC[]);;

let vUNIONS_INTERS = try Cache.lookup_thm "UNIONS_INTERS" with _ ->  prove
...
let vUNIONS_DIFF = try Cache.lookup_thm "UNIONS_DIFF" with _ ->  prove
 ((parse_term "!s t. UNIONS s DIFF t = UNIONS {x DIFF t | x IN s}"),
  vREWRITE_TAC[vUNIONS_GSPEC] ----> vSET_TAC[]);;

let vDIFF_UNIONS = try Cache.lookup_thm "DIFF_UNIONS" with _ ->  prove
 ((parse_term "!u s. u DIFF UNIONS s = u INTER INTERS {u DIFF t | t IN s}"),
  vREWRITE_TAC[vINTERS_GSPEC] ----> vSET_TAC[]);;

let vDIFF_UNIONS_NONEMPTY = try Cache.lookup_thm "DIFF_UNIONS_NONEMPTY" with _ ->  prove
 ((parse_term "!u s. ~(s = {}) ==> u DIFF UNIONS s = INTERS {u DIFF t | t IN s}"),
  vREWRITE_TAC[vINTERS_GSPEC] ----> vSET_TAC[]);;

let vINTERS_OVER_UNIONS = try Cache.lookup_thm "INTERS_OVER_UNIONS" with _ ->  prove
...
let vINTER_INTERS = try Cache.lookup_thm "INTER_INTERS" with _ ->  prove
...
let vUNIONS_OVER_INTERS = try Cache.lookup_thm "UNIONS_OVER_INTERS" with _ ->  prove
...
let vIMAGE_INTERS_SUBSET = try Cache.lookup_thm "IMAGE_INTERS_SUBSET" with _ ->  prove
 ((parse_term "!(f:A->B) g. IMAGE f (INTERS g) SUBSET INTERS (IMAGE (IMAGE f) g)"),
  vREWRITE_TAC[vINTERS_IMAGE] ----> vSET_TAC[]);;

let vIMAGE_INTER_SUBSET = try Cache.lookup_thm "IMAGE_INTER_SUBSET" with _ ->  prove
 ((parse_term "!f s t. IMAGE f (s INTER t) SUBSET IMAGE f s INTER IMAGE f t"),
  vSET_TAC[]);;

let vIMAGE_INTER_SATURATED_GEN = try Cache.lookup_thm "IMAGE_INTER_SATURATED_GEN" with _ ->  prove
 ((parse_term "!f:A->B s t u.\n        {x | x IN u /\\ f(x) IN IMAGE f s} SUBSET s /\\ t SUBSET u \\/\n        {x | x IN u /\\ f(x) IN IMAGE f t} SUBSET t /\\ s SUBSET u\n        ==> IMAGE f (s INTER t) = IMAGE f s INTER IMAGE f t"),
  vSET_TAC[]);;

let vIMAGE_INTERS_SATURATED_GEN = try Cache.lookup_thm "IMAGE_INTERS_SATURATED_GEN" with _ ->  prove
...
let vIMAGE_INTER_SATURATED = try Cache.lookup_thm "IMAGE_INTER_SATURATED" with _ ->  prove
 ((parse_term "!f:A->B s t.\n        {x | f(x) IN IMAGE f s} SUBSET s \\/ {x | f(x) IN IMAGE f t} SUBSET t\n         ==> IMAGE f (s INTER t) = IMAGE f s INTER IMAGE f t"),
  vSET_TAC[]);;

let vIMAGE_INTERS_SATURATED = try Cache.lookup_thm "IMAGE_INTERS_SATURATED" with _ ->  prove
...
let vFINITE_INDUCT_STRONG = try Cache.lookup_thm "FINITE_INDUCT_STRONG" with _ ->  prove
...
let vINJECTIVE_ON_ALT = try Cache.lookup_thm "INJECTIVE_ON_ALT" with _ ->  prove
 ((parse_term "!P f. (!x y. P x /\\ P y /\\ f x = f y ==> x = y) <=>\n         (!x y. P x /\\ P y ==> (f x = f y <=> x = y))"),
  vMESON_TAC[]);;

let vINJECTIVE_ALT = try Cache.lookup_thm "INJECTIVE_ALT" with _ ->  prove
 ((parse_term "!f. (!x y. f x = f y ==> x = y) <=> (!x y. f x = f y <=> x = y)"),
  vMESON_TAC[]);;

let vSURJECTIVE_ON_RIGHT_INVERSE = try Cache.lookup_thm "SURJECTIVE_ON_RIGHT_INVERSE" with _ ->  prove
 ((parse_term "!f t. (!y. y IN t ==> ?x. x IN s /\\ (f(x) = y)) <=>\n         (?g. !y. y IN t ==> g(y) IN s /\\ (f(g(y)) = y))"),
  vREWRITE_TAC[vRIGHT_IMP_EXISTS_THM; vSKOLEM_THM]);;

let vINJECTIVE_ON_LEFT_INVERSE = try Cache.lookup_thm "INJECTIVE_ON_LEFT_INVERSE" with _ ->  prove
...
let vBIJECTIVE_ON_LEFT_RIGHT_INVERSE = try Cache.lookup_thm "BIJECTIVE_ON_LEFT_RIGHT_INVERSE" with _ ->  prove
...
let vSURJECTIVE_RIGHT_INVERSE = try Cache.lookup_thm "SURJECTIVE_RIGHT_INVERSE" with _ ->  prove
 ((parse_term "(!y. ?x. f(x) = y) <=> (?g. !y. f(g(y)) = y)"),
  vMESON_TAC[vSURJECTIVE_ON_RIGHT_INVERSE; vIN_UNIV]);;

let vINJECTIVE_LEFT_INVERSE = try Cache.lookup_thm "INJECTIVE_LEFT_INVERSE" with _ ->  prove
...
let vBIJECTIVE_LEFT_RIGHT_INVERSE = try Cache.lookup_thm "BIJECTIVE_LEFT_RIGHT_INVERSE" with _ ->  prove
...
let vFUNCTION_FACTORS_LEFT_GEN = try Cache.lookup_thm "FUNCTION_FACTORS_LEFT_GEN" with _ ->  prove
...
let vFUNCTION_FACTORS_LEFT = try Cache.lookup_thm "FUNCTION_FACTORS_LEFT" with _ ->  prove
...
let vFUNCTION_FACTORS_RIGHT_GEN = try Cache.lookup_thm "FUNCTION_FACTORS_RIGHT_GEN" with _ ->  prove
 ((parse_term "!P f g. (!x. P x ==> ?y. g(y) = f(x)) <=>\n           (?h. !x. P x ==> f(x) = g(h x))"),
  vREWRITE_TAC[vGSYM vSKOLEM_THM] ----> vMESON_TAC[]);;

let vFUNCTION_FACTORS_RIGHT = try Cache.lookup_thm "FUNCTION_FACTORS_RIGHT" with _ ->  prove
 ((parse_term "!f g. (!x. ?y. g(y) = f(x)) <=> ?h. f = g o h"),
  vREWRITE_TAC[vFUN_EQ_THM; o_THM; vGSYM vSKOLEM_THM] ----> vMESON_TAC[]);;

let vSURJECTIVE_FORALL_THM = try Cache.lookup_thm "SURJECTIVE_FORALL_THM" with _ ->  prove
...
let vSURJECTIVE_EXISTS_THM = try Cache.lookup_thm "SURJECTIVE_EXISTS_THM" with _ ->  prove
 ((parse_term "!f:A->B. (!y. ?x. f x = y) <=> (!P. (?x. P(f x)) <=> (?y. P y))"),
  vGEN_TAC ----> vEQ_TAC ++--> [vMESON_TAC[]; vALL_TAC] ---->
  vDISCH_THEN(vMP_TAC -| vSPEC (parse_term "\\y:B. !x:A. ~(f x = y)")) ----> vMESON_TAC[]);;

let vSURJECTIVE_IMAGE_THM = try Cache.lookup_thm "SURJECTIVE_IMAGE_THM" with _ ->  prove
...
let vIMAGE_INJECTIVE_IMAGE_OF_SUBSET = try Cache.lookup_thm "IMAGE_INJECTIVE_IMAGE_OF_SUBSET" with _ ->  prove
...
let vFINITE_EMPTY = try Cache.lookup_thm "FINITE_EMPTY" with _ ->  prove
 ((parse_term "FINITE {}"),
  vREWRITE_TAC[vFINITE_RULES]);;

let vFINITE_SUBSET = try Cache.lookup_thm "FINITE_SUBSET" with _ ->  prove
...
let vFINITE_RESTRICT = try Cache.lookup_thm "FINITE_RESTRICT" with _ ->  prove
 ((parse_term "!s:A->bool P. FINITE s ==> FINITE {x | x IN s /\\ P x}"),
  vMESON_TAC[vSUBSET_RESTRICT; vFINITE_SUBSET]);;

let vFINITE_UNION_IMP = try Cache.lookup_thm "FINITE_UNION_IMP" with _ ->  prove
...
let vFINITE_UNION = try Cache.lookup_thm "FINITE_UNION" with _ ->  prove
...
let vFINITE_INTER = try Cache.lookup_thm "FINITE_INTER" with _ ->  prove
 ((parse_term "!(s:A->bool) t. FINITE s \\/ FINITE t ==> FINITE (s INTER t)"),
  vMESON_TAC[vINTER_SUBSET; vFINITE_SUBSET]);;

let vFINITE_INSERT = try Cache.lookup_thm "FINITE_INSERT" with _ ->  prove
...
let vFINITE_SING = try Cache.lookup_thm "FINITE_SING" with _ ->  prove
 ((parse_term "!a. FINITE {a}"),
  vREWRITE_TAC[vFINITE_INSERT; vFINITE_RULES]);;

let vFINITE_DELETE_IMP = try Cache.lookup_thm "FINITE_DELETE_IMP" with _ ->  prove
 ((parse_term "!(s:A->bool) x. FINITE s ==> FINITE (s DELETE x)"),
  vREPEAT vSTRIP_TAC ----> vMATCH_MP_TAC vFINITE_SUBSET ---->
  vASM_REWRITE_TAC[] ----> vASM vSET_TAC[]);;

let vFINITE_DELETE = try Cache.lookup_thm "FINITE_DELETE" with _ ->  prove
...
let vFINITE_FINITE_UNIONS = try Cache.lookup_thm "FINITE_FINITE_UNIONS" with _ ->  prove
...
let vFINITE_IMAGE_EXPAND = try Cache.lookup_thm "FINITE_IMAGE_EXPAND" with _ ->  prove
...
let vFINITE_IMAGE = try Cache.lookup_thm "FINITE_IMAGE" with _ ->  prove
 ((parse_term "!(f:A->B) s. FINITE s ==> FINITE (IMAGE f s)"),
  vREWRITE_TAC[vIMAGE; vFINITE_IMAGE_EXPAND]);;

let vFINITE_IMAGE_INJ_GENERAL = try Cache.lookup_thm "FINITE_IMAGE_INJ_GENERAL" with _ ->  prove
...
let vFINITE_FINITE_PREIMAGE_GENERAL = try Cache.lookup_thm "FINITE_FINITE_PREIMAGE_GENERAL" with _ ->  prove
...
let vFINITE_FINITE_PREIMAGE = try Cache.lookup_thm "FINITE_FINITE_PREIMAGE" with _ ->  prove
...
let vFINITE_IMAGE_INJ_EQ = try Cache.lookup_thm "FINITE_IMAGE_INJ_EQ" with _ ->  prove
...
let vFINITE_IMAGE_INJ = try Cache.lookup_thm "FINITE_IMAGE_INJ" with _ ->  prove
...
let vFINITE_IMAGE_GEN = try Cache.lookup_thm "FINITE_IMAGE_GEN" with _ ->  prove
...
let vINFINITE_IMAGE = try Cache.lookup_thm "INFINITE_IMAGE" with _ ->  prove
...
let vINFINITE_IMAGE_INJ = try Cache.lookup_thm "INFINITE_IMAGE_INJ" with _ ->  prove
 ((parse_term "!f:A->B. (!x y. (f x = f y) ==> (x = y))\n            ==> !s. INFINITE s ==> INFINITE(IMAGE f s)"),
  vMESON_TAC[vINFINITE_IMAGE]);;

let vINFINITE_NONEMPTY = try Cache.lookup_thm "INFINITE_NONEMPTY" with _ ->  prove
 ((parse_term "!s. INFINITE(s) ==> ~(s = EMPTY)"),
  vMESON_TAC[vINFINITE; vFINITE_RULES]);;

let vINFINITE_DIFF_FINITE = try Cache.lookup_thm "INFINITE_DIFF_FINITE" with _ ->  prove
...
let vSUBSET_IMAGE_INJ = try Cache.lookup_thm "SUBSET_IMAGE_INJ" with _ ->  prove
...
let vSUBSET_IMAGE = try Cache.lookup_thm "SUBSET_IMAGE" with _ ->  prove
...
let vEXISTS_SUBSET_IMAGE = try Cache.lookup_thm "EXISTS_SUBSET_IMAGE" with _ ->  prove
 ((parse_term "!P f s.\n    (?t. t SUBSET IMAGE f s /\\ P t) <=> (?t. t SUBSET s /\\ P (IMAGE f t))"),
  vREWRITE_TAC[vSUBSET_IMAGE] ----> vMESON_TAC[]);;

let vFORALL_SUBSET_IMAGE = try Cache.lookup_thm "FORALL_SUBSET_IMAGE" with _ ->  prove
 ((parse_term "!P f s. (!t. t SUBSET IMAGE f s ==> P t) <=>\n           (!t. t SUBSET s ==> P(IMAGE f t))"),
  vREWRITE_TAC[vSUBSET_IMAGE] ----> vMESON_TAC[]);;

let vEXISTS_SUBSET_IMAGE_INJ = try Cache.lookup_thm "EXISTS_SUBSET_IMAGE_INJ" with _ ->  prove
 ((parse_term "!P f s.\n    (?t. t SUBSET IMAGE f s /\\ P t) <=>\n    (?t. t SUBSET s /\\\n         (!x y. x IN t /\\ y IN t ==> (f x = f y <=> x = y)) /\\\n         P (IMAGE f t))"),
  vREWRITE_TAC[vSUBSET_IMAGE_INJ] ----> vMESON_TAC[]);;

let vFORALL_SUBSET_IMAGE_INJ = try Cache.lookup_thm "FORALL_SUBSET_IMAGE_INJ" with _ ->  prove
 ((parse_term "!P f s. (!t. t SUBSET IMAGE f s ==> P t) <=>\n           (!t. t SUBSET s /\\\n                (!x y. x IN t /\\ y IN t ==> (f x = f y <=> x = y))\n                 ==> P(IMAGE f t))"),
  vREPEAT vGEN_TAC ---->
  vONCE_REWRITE_TAC[vMESON[] (parse_term "(!t. p t) <=> ~(?t. ~p t)")] ---->
  vREWRITE_TAC[vNOT_IMP; vEXISTS_SUBSET_IMAGE_INJ; vGSYM vCONJ_ASSOC]);;

let vEXISTS_FINITE_SUBSET_IMAGE_INJ = try Cache.lookup_thm "EXISTS_FINITE_SUBSET_IMAGE_INJ" with _ ->  prove
...
let vFORALL_FINITE_SUBSET_IMAGE_INJ = try Cache.lookup_thm "FORALL_FINITE_SUBSET_IMAGE_INJ" with _ ->  prove
...
let vEXISTS_FINITE_SUBSET_IMAGE = try Cache.lookup_thm "EXISTS_FINITE_SUBSET_IMAGE" with _ ->  prove
...
let vFORALL_FINITE_SUBSET_IMAGE = try Cache.lookup_thm "FORALL_FINITE_SUBSET_IMAGE" with _ ->  prove
 ((parse_term "!P f s. (!t. FINITE t /\\ t SUBSET IMAGE f s ==> P t) <=>\n           (!t. FINITE t /\\ t SUBSET s ==> P(IMAGE f t))"),
  vREPEAT vGEN_TAC ---->
  vONCE_REWRITE_TAC[vMESON[] (parse_term "(!x. P x) <=> ~(?x. ~P x)")] ---->
  vREWRITE_TAC[vNOT_IMP; vGSYM vCONJ_ASSOC; vEXISTS_FINITE_SUBSET_IMAGE]);;

let vFINITE_SUBSET_IMAGE = try Cache.lookup_thm "FINITE_SUBSET_IMAGE" with _ ->  prove
...
let vFINITE_SUBSET_IMAGE_IMP = try Cache.lookup_thm "FINITE_SUBSET_IMAGE_IMP" with _ ->  prove
 ((parse_term "!f:A->B s t.\n        FINITE(t) /\\ t SUBSET (IMAGE f s)\n        ==> ?s'. FINITE s' /\\ s' SUBSET s /\\ t SUBSET (IMAGE f s')"),
  vMESON_TAC[vSUBSET_REFL; vFINITE_SUBSET_IMAGE]);;

let vFINITE_IMAGE_EQ = try Cache.lookup_thm "FINITE_IMAGE_EQ" with _ ->  prove
...
let vFINITE_IMAGE_EQ_INJ = try Cache.lookup_thm "FINITE_IMAGE_EQ_INJ" with _ ->  prove
 ((parse_term "!(f:A->B) s. FINITE(IMAGE f s) <=>\n                ?t. FINITE t /\\ t SUBSET s /\\ IMAGE f s = IMAGE f t /\\\n                    (!x y. x IN t /\\ y IN t ==> (f x = f y <=> x = y))"),
...
let vFINITE_DIFF = try Cache.lookup_thm "FINITE_DIFF" with _ ->  prove
 ((parse_term "!s t. FINITE s ==> FINITE(s DIFF t)"),
  vMESON_TAC[vFINITE_SUBSET; vSUBSET_DIFF]);;

let vINFINITE_SUPERSET = try Cache.lookup_thm "INFINITE_SUPERSET" with _ ->  prove
 ((parse_term "!s t. INFINITE s /\\ s SUBSET t ==> INFINITE t"),
  vREWRITE_TAC[vINFINITE] ----> vMESON_TAC[vFINITE_SUBSET]);;

let vFINITE_TRANSITIVITY_CHAIN = try Cache.lookup_thm "FINITE_TRANSITIVITY_CHAIN" with _ ->  prove
...
let vUNIONS_MAXIMAL_SETS = try Cache.lookup_thm "UNIONS_MAXIMAL_SETS" with _ ->  prove
...
let vFINITE_SUBSET_UNIONS = try Cache.lookup_thm "FINITE_SUBSET_UNIONS" with _ ->  prove
...
let vUNIONS_IN_CHAIN = try Cache.lookup_thm "UNIONS_IN_CHAIN" with _ ->  prove
...
let vINTERS_IN_CHAIN = try Cache.lookup_thm "INTERS_IN_CHAIN" with _ ->  prove
 ((parse_term "!f:(A->bool)->bool.\n        FINITE f /\\ ~(f = {}) /\\\n        (!s t. s IN f /\\ t IN f ==> s SUBSET t \\/ t SUBSET s)\n        ==> INTERS f IN f"),
  vREWRITE_TAC[vIMP_CONJ] ----> vMATCH_MP_TAC vFINITE_INDUCT ---->
...
let vFINITE_SUBSET_UNIONS_CHAIN = try Cache.lookup_thm "FINITE_SUBSET_UNIONS_CHAIN" with _ ->  prove
...
let vFINREC_1_LEMMA = try Cache.lookup_thm "FINREC_1_LEMMA" with _ ->  prove
 ((parse_term "!f b s a. FINREC f b s a (SUC 0) <=> ?x. (s = {x}) /\\ (a = f x b)"),
  vREWRITE_TAC[vFINREC] ---->
  vREPEAT vGEN_TAC ----> vAP_TERM_TAC ----> vABS_TAC ----> vSET_TAC[]);;

let vFINREC_SUC_LEMMA = try Cache.lookup_thm "FINREC_SUC_LEMMA" with _ ->  prove
...
let vFINREC_UNIQUE_LEMMA = try Cache.lookup_thm "FINREC_UNIQUE_LEMMA" with _ ->  prove
...
let vFINREC_EXISTS_LEMMA = try Cache.lookup_thm "FINREC_EXISTS_LEMMA" with _ ->  prove
...
let vFINREC_FUN_LEMMA = try Cache.lookup_thm "FINREC_FUN_LEMMA" with _ ->  prove
...
let vFINREC_FUN = try Cache.lookup_thm "FINREC_FUN" with _ ->  prove
...
let vSET_RECURSION_LEMMA = try Cache.lookup_thm "SET_RECURSION_LEMMA" with _ ->  prove
...
let vFINITE_RECURSION = try Cache.lookup_thm "FINITE_RECURSION" with _ ->  prove
...
let vFINITE_RECURSION_DELETE = try Cache.lookup_thm "FINITE_RECURSION_DELETE" with _ ->  prove
...
let vITSET_EQ = try Cache.lookup_thm "ITSET_EQ" with _ ->  prove
...
let vCARD_CLAUSES = try Cache.lookup_thm "CARD_CLAUSES" with _ ->  prove
...
let vCARD_UNION = try Cache.lookup_thm "CARD_UNION" with _ ->  prove
...
let vCARD_DELETE = try Cache.lookup_thm "CARD_DELETE" with _ ->  prove
...
let vCARD_UNION_EQ = try Cache.lookup_thm "CARD_UNION_EQ" with _ ->  prove
 ((parse_term "!s t u. FINITE u /\\ (s INTER t = {}) /\\ (s UNION t = u)\n           ==> (CARD s + CARD t = CARD u)"),
  vMESON_TAC[vCARD_UNION; vFINITE_SUBSET; vSUBSET_UNION]);;

let vCARD_DIFF = try Cache.lookup_thm "CARD_DIFF" with _ ->  prove
...
let vCARD_EQ_0 = try Cache.lookup_thm "CARD_EQ_0" with _ ->  prove
 ((parse_term "!s. FINITE s ==> ((CARD s = 0) <=> (s = {}))"),
  vMATCH_MP_TAC vFINITE_INDUCT_STRONG ---->
  vSIMP_TAC[vCARD_CLAUSES; vNOT_INSERT_EMPTY; vNOT_SUC]);;

let vCARD_SING = try Cache.lookup_thm "CARD_SING" with _ ->  prove
...
let vFINITE_INDUCT_DELETE = try Cache.lookup_thm "FINITE_INDUCT_DELETE" with _ ->  prove
...
let vHAS_SIZE_CARD = try Cache.lookup_thm "HAS_SIZE_CARD" with _ ->  prove
 ((parse_term "!s n. s HAS_SIZE n ==> (CARD s = n)"),
  vSIMP_TAC[vHAS_SIZE]);;

let vHAS_SIZE_0 = try Cache.lookup_thm "HAS_SIZE_0" with _ ->  prove
...
let vHAS_SIZE_SUC = try Cache.lookup_thm "HAS_SIZE_SUC" with _ ->  prove
...
let vHAS_SIZE_UNION = try Cache.lookup_thm "HAS_SIZE_UNION" with _ ->  prove
...
let vHAS_SIZE_DIFF = try Cache.lookup_thm "HAS_SIZE_DIFF" with _ ->  prove
 ((parse_term "!s t m n. s HAS_SIZE m /\\ t HAS_SIZE n /\\ t SUBSET s\n             ==> (s DIFF t) HAS_SIZE (m - n)"),
  vSIMP_TAC[vHAS_SIZE; vFINITE_DIFF; vCARD_DIFF]);;

let vHAS_SIZE_UNIONS = try Cache.lookup_thm "HAS_SIZE_UNIONS" with _ ->  prove
...
let vFINITE_HAS_SIZE = try Cache.lookup_thm "FINITE_HAS_SIZE" with _ ->  prove
...
let vHAS_SIZE_CLAUSES = try Cache.lookup_thm "HAS_SIZE_CLAUSES" with _ ->  prove
...
let vCARD_SUBSET_EQ = try Cache.lookup_thm "CARD_SUBSET_EQ" with _ ->  prove
...
let vCARD_SUBSET = try Cache.lookup_thm "CARD_SUBSET" with _ ->  prove
...
let vCARD_PSUBSET_IMP = try Cache.lookup_thm "CARD_PSUBSET_IMP" with _ ->  prove
 ((parse_term "!a b. a SUBSET b /\\ ~(CARD a = CARD b) ==> a PSUBSET b"),
  vREWRITE_TAC[vPSUBSET] ----> vMESON_TAC[]);;

let vCARD_PSUBSET_EQ = try Cache.lookup_thm "CARD_PSUBSET_EQ" with _ ->  prove
 ((parse_term "!a b. FINITE b /\\ a SUBSET b ==> (a PSUBSET b <=> CARD a < CARD b)"),
  vMESON_TAC[vCARD_PSUBSET_IMP; vCARD_PSUBSET; vLT_REFL]);;

let vCARD_UNION_LE = try Cache.lookup_thm "CARD_UNION_LE" with _ ->  prove
...
let vFINITE_CARD_LE_UNION = try Cache.lookup_thm "FINITE_CARD_LE_UNION" with _ ->  prove
 ((parse_term "!s (t:A->bool) m n.\n        (FINITE s /\\ CARD s <= m) /\\\n        (FINITE t /\\ CARD t <= n)\n        ==> FINITE(s UNION t) /\\ CARD(s UNION t) <= m + n"),
  vMESON_TAC[vFINITE_UNION; vCARD_UNION_LE; vLE_ADD2; vLE_TRANS]);;

let vCARD_UNIONS_LE = try Cache.lookup_thm "CARD_UNIONS_LE" with _ ->  prove
...
let vCARD_UNION_GEN = try Cache.lookup_thm "CARD_UNION_GEN" with _ ->  prove
...
let vCARD_UNION_OVERLAP_EQ = try Cache.lookup_thm "CARD_UNION_OVERLAP_EQ" with _ ->  prove
...
let vCARD_UNION_OVERLAP = try Cache.lookup_thm "CARD_UNION_OVERLAP" with _ ->  prove
...
let vCARD_IMAGE_INJ = try Cache.lookup_thm "CARD_IMAGE_INJ" with _ ->  prove
...
let vHAS_SIZE_IMAGE_INJ = try Cache.lookup_thm "HAS_SIZE_IMAGE_INJ" with _ ->  prove
...
let vCARD_IMAGE_LE = try Cache.lookup_thm "CARD_IMAGE_LE" with _ ->  prove
...
let vFINITE_CARD_LE_IMAGE = try Cache.lookup_thm "FINITE_CARD_LE_IMAGE" with _ ->  prove
 ((parse_term "!(f:A->B) s n.\n        FINITE s /\\ CARD s <= n ==> FINITE(IMAGE f s) /\\ CARD(IMAGE f s) <= n"),
  vMESON_TAC[vCARD_IMAGE_LE; vLE_TRANS; vFINITE_IMAGE]);;

let vCARD_IMAGE_INJ_EQ = try Cache.lookup_thm "CARD_IMAGE_INJ_EQ" with _ ->  prove
 ((parse_term "!f:A->B s t.\n        FINITE s /\\\n        (!x. x IN s ==> f(x) IN t) /\\\n        (!y. y IN t ==> ?!x. x IN s /\\ f(x) = y)\n        ==> CARD t = CARD s"),
  vREPEAT vSTRIP_TAC ---->
...
let vCARD_SUBSET_IMAGE = try Cache.lookup_thm "CARD_SUBSET_IMAGE" with _ ->  prove
 ((parse_term "!f s t. FINITE t /\\ s SUBSET IMAGE f t ==> CARD s <= CARD t"),
  vMESON_TAC[vLE_TRANS; vFINITE_IMAGE; vCARD_IMAGE_LE; vCARD_SUBSET]);;

let vHAS_SIZE_IMAGE_INJ_EQ = try Cache.lookup_thm "HAS_SIZE_IMAGE_INJ_EQ" with _ ->  prove
...
let vCARD_IMAGE_EQ_INJ = try Cache.lookup_thm "CARD_IMAGE_EQ_INJ" with _ ->  prove
...
let vEXISTS_SMALL_SUBSET_IMAGE_INJ = try Cache.lookup_thm "EXISTS_SMALL_SUBSET_IMAGE_INJ" with _ ->  prove
...
let vFORALL_SMALL_SUBSET_IMAGE_INJ = try Cache.lookup_thm "FORALL_SMALL_SUBSET_IMAGE_INJ" with _ ->  prove
 ((parse_term "!P f s n.\n    (!t. FINITE t /\\ CARD t < n /\\ t SUBSET IMAGE f s ==> P t) <=>\n    (!t. FINITE t /\\ CARD t < n /\\ t SUBSET s /\\\n         (!x y. x IN t /\\ y IN t ==> (f x = f y <=> x = y))\n         ==> P (IMAGE f t))"),
  vREPEAT vGEN_TAC ---->
  vONCE_REWRITE_TAC[vMESON[] (parse_term "(!t. p t) <=> ~(?t. ~p t)")] ---->
  vREWRITE_TAC[vNOT_IMP; vEXISTS_SMALL_SUBSET_IMAGE_INJ; vGSYM vCONJ_ASSOC]);;

let vEXISTS_SMALL_SUBSET_IMAGE = try Cache.lookup_thm "EXISTS_SMALL_SUBSET_IMAGE" with _ ->  prove
 ((parse_term "!P f s n.\n    (?t. FINITE t /\\ CARD t < n /\\ t SUBSET IMAGE f s /\\ P t) <=>\n    (?t. FINITE t /\\ CARD t < n /\\ t SUBSET s /\\\n         P (IMAGE f t))"),
  vREPEAT vGEN_TAC ----> vEQ_TAC ++-->
   [vREWRITE_TAC[vEXISTS_SMALL_SUBSET_IMAGE_INJ] ----> vMESON_TAC[];
    vMESON_TAC[vFINITE_IMAGE; vCARD_IMAGE_LE; vLET_TRANS; vIMAGE_SUBSET]]);;

let vFORALL_SMALL_SUBSET_IMAGE = try Cache.lookup_thm "FORALL_SMALL_SUBSET_IMAGE" with _ ->  prove
...
let vCHOOSE_SUBSET_STRONG = try Cache.lookup_thm "CHOOSE_SUBSET_STRONG" with _ ->  prove
...
let vCHOOSE_SUBSET_EQ = try Cache.lookup_thm "CHOOSE_SUBSET_EQ" with _ ->  prove
 ((parse_term "!n s:A->bool.\n     (FINITE s ==> n <= CARD s) <=> (?t. t SUBSET s /\\ t HAS_SIZE n)"),
...
let vCHOOSE_SUBSET = try Cache.lookup_thm "CHOOSE_SUBSET" with _ ->  prove
 ((parse_term "!s:A->bool. FINITE s ==> !n. n <= CARD s ==> ?t. t SUBSET s /\\ t HAS_SIZE n"),
  vMESON_TAC[vCHOOSE_SUBSET_STRONG]);;

let vCHOOSE_SUBSET_BETWEEN = try Cache.lookup_thm "CHOOSE_SUBSET_BETWEEN" with _ ->  prove
...
let vCARD_LE_UNIONS_CHAIN = try Cache.lookup_thm "CARD_LE_UNIONS_CHAIN" with _ ->  prove
...
let vCARD_LE_1 = try Cache.lookup_thm "CARD_LE_1" with _ ->  prove
...
let vINVOLUTION_EVEN_NOFIXPOINTS = try Cache.lookup_thm "INVOLUTION_EVEN_NOFIXPOINTS" with _ ->  prove
...
let vINVOLUTION_EVEN_FIXPOINTS = try Cache.lookup_thm "INVOLUTION_EVEN_FIXPOINTS" with _ ->  prove
