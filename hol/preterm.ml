...
let overload_interface (sym,tm) =
  ...
  let (_name,ty) as namty = try dest_const tm with Failure _ -> dest_var tm in
...
let prioritize_overload ty =
  do_list
   (fun (s,gty) ->
      try let _,(n,t) = find
            (fun (s',(_n,t)) -> s' = s && mem ty (map fst (type_match gty t [])))
            (!the_interface) in
...
let type_of_pretype,term_of_preterm,retypecheck =
  ...
  let pmk_numeral =
    ...
    let vNUMERAL = Constp("NUMERAL",Ptycon("fun",[num_pty; num_pty]))
    and vBIT0 = Constp("BIT0",Ptycon("fun",[num_pty; num_pty]))
    and vBIT1 = Constp("BIT1",Ptycon("fun",[num_pty; num_pty]))
    and t_0 = Constp("_0",num_pty) in
    let rec pmk_numeral(n) =
      if n =/ num_0 then t_0 else
      let m = quo_num n (num_2) and b = mod_num n (num_2) in
      let op = if b =/ num_0 then vBIT0 else vBIT1 in
      Combp(op,pmk_numeral(m)) in
    fun n -> Combp(vNUMERAL,pmk_numeral n) in
  ...
  let rec istrivial ptm env x = function
    ...
    |Ptycon(_f,args) when exists (istrivial ptm env x) args ->
  ...
  let unify ptm env ty1 ty2 =
    ...
    |(_,_,ptm)::_oth -> failwith (string_of_ty_error env ptm)
    in
  ...
  let term_of_preterm =
    let rec term_of_preterm ptm =
      ...
      | Typing(ptm,_pty) -> term_of_preterm ptm in
