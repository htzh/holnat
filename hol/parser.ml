...
let lex =
  ...
  let rawtoken = (string ||| some isbra ||| septok ||| ident) >>
    (fun x -> Ident x) in
  let simptoken = many (some isspace) ++ rawtoken >> (reserve -| snd) in
  let rec tokens i =
...
(*                                                                           *)
(*  -| Any identifier not in use as a type constant will be parsed as a       *)
...
let parse_pretype =
  let mk_prefinty:num->pretype =
    ...
    fun n ->
      ...
      prefinty n in
  let btyop n _n' x y = Ptycon(n,[x;y])
  and mk_apptype =
    function
        ([s],[]) -> s
      | (tys,[c]) -> Ptycon(c,tys)
  ...
  and atomictypes i =
        (((a (Resword "(")) ++ typelist ++ a (Resword ")") >> (snd -| fst))
      ||| (type_atom >> (fun x -> [x]))) i
...
let parse_preterm =
  ...
  let pdest_eq = function | (Combp(Combp(Varp(("="|"<=>"),_),l),r)) -> l,r | _ -> assert false in
  ...
  let rec mk_precedence infxs prs inp =
    match infxs with
      (_s,(p,at))::_ ->
          let topins,rest = partition (fun (_s',pat') -> pat' = (p,at)) infxs in
          (if at = "right" then rightbin else leftbin)
          (mk_precedence rest prs)
          (end_itlist (|||) (map (fun (s,_) -> a (Ident s)) topins))
          (fun oid x y -> match oid with (Ident op) -> Combp(Combp(Varp(op,dpty),x),y) | _ -> assert false)
  ...
  and binder_preterm i =
   ((a (Resword "let") ++
     leftbin (preterm >> singleton1) (a (Resword "and")) (vK (@)) "binding" ++
     a (Resword "in") ++
     preterm
    >> lmk_let)
  ||| (binder ++
...
let parse_type s =
  let pty,l = (parse_pretype -| lex -| explode) s in
  if l = [] then type_of_pretype pty
  else failwith "Unparsed input following type";;

let parse_term s =
  let ptm,l = (parse_preterm -| lex -| explode) s in
  if l = [] then
   (term_of_preterm -| (retypecheck [])) ptm
