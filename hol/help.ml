...
let help s =
  ...
  (if mem s mod_listing then
    ...
    (Format.print_string -| end_itlist(^))
...
let omit t = mk_comb(mk_var("<omit this pattern>",vW mk_fun_ty (type_of t)),t);;

let exactly t = mk_comb(mk_var("<match aconv>",vW mk_fun_ty (type_of t)),t);;

let name s = mk_comb(mk_var("<match theorem name>",vW mk_fun_ty aty),
                     mk_var(s,aty));;
...
let search =
  ...
  let rec sublist l1 l2 =
    match (l1,l2) with
      [],_ -> true
    | _,[] -> false
    | (_h1::_t1,_h2::t2) -> immediatesublist l1 l2 || sublist l1 t2 in
  let exists_subterm_satisfying p (_n,th) = can (find_term p) (concl th)
  and name_contains s (n,_th) = sublist (explode s) (explode n) in
  let rec filterpred tm =
    match tm with
      Comb(Var("<omit this pattern>",_),t) -> not -| filterpred t
    ...
    (if triv <> [] then
      warn true
         ("Ignoring plain variables in search: "^
          end_itlist (fun s t -> s^", "^t) (map (fst -| dest_var) triv))
     else ());
    (if nontriv = [] && triv <> [] then []
     else itlist (filter -| filterpred) pats (!theorems));;
