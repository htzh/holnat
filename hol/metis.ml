...
let lowerPriorityNode node1 node2 =
        let {priority = p1; _} = node1
      and {priority = p2; _} = node2
    in
    ...
    | Tree node1 ->
      ...
      | Tree node2 ->
        if lowerPriorityNode node1 node2 then
            let {priority=priority;left=left;key=key;value=value;right=right; _} = node2

            in let left = treeAppend tree1 left
          in
            mkTree priority left key value right
        else
            let {priority=priority;left=left;key=key;value=value;right=right; _} = node1
...
and nodePeek compareKey pkey node =
      let {left=left;key=key;value=value;right=right; _} = node
    in
...
and nodePeekPath compareKey pkey path node =
      let {left=left;key=key;right=right; _} = node
    in
      match compareKey (pkey,key) with
...
let addSidePath ((wentLeft,node),(leftTree,rightTree)) =
      let {priority=priority;left=left;key=key;value=value;right=right; _} = node
    in
...
let treeInsert compareKey key_value tree =
    ...
    in
      ...
      | Some node ->
          let {size=size;priority=priority;left=left;right=right; _} = node
...
and nodeMapPartial f ({priority=priority;left=left;key=key;value=value;right=right; _}) =
...
and nodeIntersect compareKey f n1 n2 =
      let {priority=priority;left=left;key=key;value=value;right=right; _} = n2
...
and nodeDifferenceDomain compareKey n1 n2 =
    if pointerEqual (n1,n2) then Empty
        ...
        let {priority=priority;left=left;key=key;value=value;right=right; _} = n1
...
and nodeSubsetDomain compareKey node1 node2 =
    pointerEqual (node1,node2) ||
      let {size=size;left=left;key=key;right=right; _} = node1
    in
      size <= nodeSize node2 &&
...
let nodePick node =
      let {key=key;value=value; _} = node
    in
      (key,value)
    ;;
...
let symbols tm =
  let rec sz n = function
    ...
    | (Fn (_letc,args) :: tms) -> sz (n + fN_SYMBOLS) (args @ tms)
  in sz 0 [tm];;
...
let toString (Thm (cl, (infType, _ths))) =
...
let lookupTables tables (n,elts) =
      let {domainSize = vN; rangeSize = vR; _} = tables
...
let msize ({sizem = vN; _}) = vN;;
let psize ({sizep = vN; _}) = vN;;

let peekFixedFunction vM (n,elts) =
      let {fixedFunctions = fixFns; _} = vM
    in
...
let peekFixedRelation vM (n,elts) =
      let {fixedRelations = fixRels; _} = vM
    in
...
let interpretFunction vM n_elts =
    match peekFixedFunction vM n_elts with
      Some r -> r
    | None ->
        let {randomFunctions = rndFns; _} = vM
      in
...
let interpretRelation vM n_elts =
    match peekFixedRelation vM n_elts with
      Some r -> r
    | None ->
        let {randomRelations = rndRels; _} = vM
      in
...
let newNet parm = {positive = Atom_net.newNet parm; negative = Atom_net.newNet parm};;
  ...
  let pos ({positive=positive; _} : 'a literalNet) = Atom_net.size positive;;

  let neg ({negative=negative; _} : 'a literalNet) = Atom_net.size negative;;
...
let size ({empty=empty; unitn=unitn; nonunit = {clauses=clauses; _}}) =
...
let deleteWaiting (Rewrite {waiting=waiting; _} as rw) id =
...
let peek (Rewrite {known=known; _}) id = Intmap.peek known id;;

let size (Rewrite {known=known; _}) = Intmap.size known;;

let equations (Rewrite {known=known; _}) =
...
let add (Rewrite {known=known; _} as rw) (id,eqn) =
    if Intmap.inDomain id known then rw
    else
        let Rewrite {order=order;redexes=redexes;subterms=subterms;waiting=waiting; _} = rw
...
let rec reduceAcc (rpl, spl, todo, (Rewrite {known=known;waiting=waiting; _} as rw), changed) =
...
let isReduced (Rewrite {waiting=waiting; _}) = Intset.null waiting;;
...
let toString (Clause {id=_id;thm=thm; _}) = Thm.toString thm;;
...
let id (Clause {id = i; _}) = i;;

let thm (Clause {thm = th; _}) = th;;
...
let isTautology (Clause {thm=thm; _}) = Thm.isTautology thm;;

let isContradiction (Clause {thm=thm; _}) = Thm.isContradiction thm;;
...
let isLargerTerm ({ordering=ordering;orderTerms=orderTerms; _} : parameters) l_r =
  ...
  let isLargerLiteral ({ordering=ordering;orderLiterals=orderLiterals; _} : parameters) lits =
...
let largestLiterals (Clause {parameters=parameters;thm=thm; _}) =
...
let largestEquations (Clause {parameters=parameters; _} as cl) =
...
let rewrite rewr (Clause {parameters=parameters;id=id;thm=thm}) =
      let simp th =
            let {ordering=ordering; _} = parameters
...
let factor (Clause {parameters=parameters;thm=thm; _} as cl) =
...
*)
      let Clause {parameters=parameters; thm = th1; _} = cl1
      and Clause {thm = th2; _} = cl2
...
let getSubsume (Active {subsume = s; _}) = s;;
...
let setRewrite active rewrite =
      let Active
            {parameters=parameters;clauses=clauses;units=units;subsume=subsume;literals=literals;equations=equations;
             subterms=subterms;allSubterms=allSubterms; _} = active
    in
      Active
...
let empty parameters =
      let {clause=clause; _} = parameters
      in let {Clause.ordering=ordering; _} = clause
    in
      Active
        {parameters = parameters;
         clauses = Intmap.newMap ();
...
let size (Active {clauses=clauses; _}) = Intmap.size clauses;;

let clauses (Active {clauses = cls; _}) =
...
let saturation active =
      ...
      in let (cls,_subs) = Mlist.foldl remove ([], Subsume.newSubsume ()) cls
...
let simplifyActive simp active =
      let Active {units=units;rewrite=rewrite;subsume=subsume; _} = active
    in
...
let deduce active cl =
      let Active {parameters=_parameters;literals=literals;equations=equations;subterms=subterms; _} = active
...
(* ------------------------------------------------------------------------- *)
  ...
  let clause_rewritables active =
        let Active {clauses=clauses;rewrite=rewrite; _} = active
  ...
  let rewrite_rewritables active rewr_ids =
        let Active {parameters=parameters;rewrite=rewrite;clauses=clauses;allSubterms=allSubterms; _} = active
        in let {clause = {Clause.ordering=ordering; _}; _} = parameters
...
*)
  ...
  let extract_rewritables (Active {clauses=clauses;rewrite=rewrite; _} as active) =
...
(* ------------------------------------------------------------------------- *)
  ...
  let prefactor_simplify active subsume =
        let Active {parameters=parameters;units=units;rewrite=rewrite; _} = active
        in let {prefactor=prefactor; _} = parameters
      in
  ...
  let postfactor_simplify active subsume =
        let Active {parameters=parameters;units=units;rewrite=rewrite; _} = active
        in let {postfactor=postfactor; _} = parameters
      in
...
let size (Waiting {clauses=clauses; _}) = Heap.size clauses;;
...
let initialModel axioms conjecture parm =
      let {model=model;initialPerturbations=initialPerturbations; _}  = parm
...
let checkModels parms models (fv,cl) =
      let check ((parm,model),z) =
            let {maxChecks=maxChecks;weight=weight; _} = parm
            in let n = maxChecks
...
let perturbModels parms models cls =
      let perturb (parm,model) =
            let {perturbations=perturbations; _} = parm
          in
...
let add' waiting dist mcls cls =
      ...
      in let {modelsP = modelParameters; _} = parameters
...
*)
  ...
  let empty parameters axioms conjecture =
        let {modelsP = modelParameters; _} = parameters
...
let active (Resolution {active = a; _}) = a;;

let waiting (Resolution {waiting = w; _}) = w;;
...
let mk_negp pt = Combp (preterm_of_term (parse_term "~"), pt)
let mk_eqp (ps, pt) = Combp (Combp (Constp ("=", dpty), ps), pt)
let mk_conjp (ps, pt) = Combp (Combp (preterm_of_term (parse_term "/\\"), ps), pt)
let mk_disjp (ps, pt) = Combp (Combp (preterm_of_term (parse_term "\\/"), ps), pt)
...
let rec env_of_preterm = function
  ...
  | Typing (ptm, _pty) -> env_of_preterm ptm
  | _ -> failwith "env_of_preterm"

let env_of_th = env_of_preterm -| preterm_of_term -| concl
let env_of_ths = List.concat -| List.map env_of_th
...
let preterm_of_const = preterm_of_term -| hol_of_const -| int_of_string
...
let typecheck env = term_of_preterm -| retypecheck env -| Preterm.unconst_preterm
let typecheckl env = function
    [] -> []
  | xs -> Preterm.list_mk_disjp xs |> typecheck env |> disjuncts


let hol_of_term env = typecheck env -| preterm_of_fol_term

let hol_of_atom env = typecheck env -| preterm_of_atom

let hol_of_literal env = typecheck env -| preterm_of_literal

let hol_of_clause env = typecheck env -| Preterm.list_mk_disjp -| map preterm_of_literal

let hol_of_substitution env = map dest_eq -| typecheckl env -| map preterm_of_eq
...
let vFRONT lit thm =
  ...
  let eq = vDISJ_ACI_RULE (mk_eq (conc, conc')) in
  (vPURE_ONCE_REWRITE_RULE [eq] thm, rest)
...
let vRESOLVE_N =
  let vRESOLVE_1  = vTAUT (parse_term "!a. a ==> ~a ==> F")
  and vRESOLVE_2L = vTAUT (parse_term "!a b. a \\/ b ==> ~a ==> b")
  and vRESOLVE_2R = vTAUT (parse_term "!a c. a ==> ~a \\/ c ==> c")
  and vRESOLVE_3  = vTAUT (parse_term "!a b c. a \\/ b ==> ~a \\/ c ==> b \\/ c") in
  fun atom -> function
  ([], []) -> vSPEC atom vRESOLVE_1
| (r1, []) -> vSPECL [atom; list_mk_disj r1] vRESOLVE_2L
| ([], r2) -> vSPECL [atom; list_mk_disj r2] vRESOLVE_2R
| (r1, r2) -> vSPECL [atom; list_mk_disj r1; list_mk_disj r2] vRESOLVE_3

(* resolve two clauses th1 and th2, where atom appears somewhere
   positive in th1 and negative in th2 *)
let vRESOLVE atom th1 th2 =
  ...
  try let (th1', r1) = vFRONT atom th1
  and (th2', r2) = vFRONT (mk_neg atom) th2 in
  let res = vRESOLVE_N atom (r1, r2) in
  vMP (vMP res th1') th2'
...
let vDISCH_DISJ =
  let vIMPL_NOT_L = vTAUT (parse_term "!a b. ~a ==> b <=>  a \\/ b")
  and vIMPL_NOT_R = vTAUT (parse_term "!a b.  a ==> b <=> ~a \\/ b") in
  fun tm th ->
    let impl = vDISCH tm th
    and (tm', vIMPL_NOT) =
      try dest_neg tm, vIMPL_NOT_L
      with _ ->    tm, vIMPL_NOT_R in
    let eq = vSPECL [tm'; concl th] vIMPL_NOT in
    vPURE_ONCE_REWRITE_RULE [eq] impl

(* given A, tm1, .., tmn |- th, prove A |- ~tm1 \/ .. \/ ~tmn \/ th *)
let vDISCH_DISJS tms th = List.fold_right vDISCH_DISJ tms th
...
let string_of_tminst = String.concat ", " -|
  map (fun (tm, v) -> string_of_term tm ^ " <- " ^ string_of_term v)

let string_of_tyinst = String.concat ", " -|
  map (fun (ty, v) -> string_of_type ty ^ " <- " ^ string_of_type v)

let string_of_instantiation (_it, tminst, tyinst) =
...
let rec hol_of_thm axioms fth =
  ...
  | Proof.Subst (fsub, fth1) ->
      ...
      vINSTANTIATE ([], tminst, tyinst) th1
  | Proof.Resolve (atom, fth1, fth2) ->
      ...
      let tm1 = striplist dest_disj (concl th1) |> List.filter (not -| is_neg)
      ...
      Metis_rules.vRESOLVE (inst tyinsts hatom)
        (vINST_TYPE tyinsts th1) (vINST_TYPE tyinsts th2)
  | Proof.Refl term -> vREFL (Metis_mapping.hol_of_term env term)
  | Proof.Assume atom -> vSPEC (Metis_mapping.hol_of_atom env atom) vEXCLUDED_MIDDLE
  | Proof.Equality (flit, fpath, ft) ->
      let hlit = Metis_mapping.hol_of_literal env flit in
      let _fs, hpath = Metis_path.hol_of_literal_path flit fpath in
      ...
      let conv = vPATH_CONV hpath (vPURE_ONCE_REWRITE_CONV [vASSUME heq]) in
      let hlit' = vCONV_RULE conv (vASSUME hlit) in
      if !metisverb then Format.printf "hlit = %s, hlit' = %s\n%!"
        (string_of_term hlit) (string_of_thm hlit');

      if hs <> ht then assert (concl hlit' <> hlit);
      (try Metis_rules.vDISCH_DISJS [heq; hlit] hlit'
      with _ -> failwith "equality")
  in
    (* eliminate duplicates in clause *)
    let hth = vCONV_RULE vDISJ_CANON_CONV hth in
    if !metisverb then begin
...
(* Debugging tactic. *)
let vPRINT_TAC g = print_goal g; vALL_TAC g
let vPRINT_ID_TAC s g = print_endline s; vPRINT_TAC g

(* Slightly modified tactic from meson.ml. *)
let vFOL_PREPARE_TAC ths =
...
let vSIMPLE_METIS_REFUTE ths =
  ...
  let ths = map (vCONV_RULE vDISJ_CANON_CONV) ths in
  ...
  assert (concl proof = (parse_term "F"));
  proof

let vPURE_METIS_TAC g =
  Meson.reset_vars(); Meson.reset_consts();
  (vFIRST_ASSUM vCONTR_TAC |--->
   vW(vACCEPT_TAC -| vSIMPLE_METIS_REFUTE -| map snd -| fst)) g

let vGEN_METIS_TAC ths =
  vFOL_PREPARE_TAC ths ----> vPURE_METIS_TAC
...
let vASM_METIS_TAC = Metis.vGEN_METIS_TAC;;

let vMETIS_TAC ths = vPOP_ASSUM_LIST(vK vALL_TAC) ----> vASM_METIS_TAC ths;;

let vMETIS ths tm = prove(tm,vMETIS_TAC ths);;
