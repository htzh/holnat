...
module type Hol_kernel =
  sig
      ...
      val concl : thm -> term
      val vREFL : term -> thm
      val vTRANS : thm -> thm -> thm
      val vMK_COMB : thm * thm -> thm
      val vABS : term -> thm -> thm
      val vBETA : term -> thm
      val vASSUME : term -> thm
      val vEQ_MP : thm -> thm -> thm
      val vDEDUCT_ANTISYM_RULE : thm -> thm -> thm
      val vINST_TYPE : (hol_type * hol_type) list -> thm -> thm
      val vINST : (term * term) list -> thm -> thm
...
(* ------------------------------------------------------------------------- *)
  ...
  let rec type_of tm =
    match tm with
      Var(_,ty) -> ty
    | Const(_,ty) -> ty
    | Comb(s,_) -> (match type_of s with Tyapp("fun",[_dty;rty]) -> rty | _ -> assert false)
...
(* ------------------------------------------------------------------------- *)
  ...
  let vsubst =
    let rec vsubst ilist tm =
      ...
      | Abs(v,s) -> let ilist' = filter (fun (_t,x) -> x <> v) ilist in
...
(* ------------------------------------------------------------------------- *)
  ...
  let rec orda env tm1 tm2 =
    ...
    match (tm1,tm2) with
      Var(_x1,_ty1),Var(_x2,_ty2) -> ordav env tm1 tm2
    | Const(_x1,_ty1),Const(_x2,_ty2) -> Stdlib.compare tm1 tm2
...
(* ------------------------------------------------------------------------- *)
  ...
  let hyp (Sequent(asl,_c)) = asl

  let concl (Sequent(_asl,c)) = c
...
(* ------------------------------------------------------------------------- *)
  ...
  let vTRANS (Sequent(asl1,c1)) (Sequent(asl2,c2)) =
    match (c1,c2) with
...
(* ------------------------------------------------------------------------- *)
  ...
  let vMK_COMB(Sequent(asl1,c1),Sequent(asl2,c2)) =
     match (c1,c2) with
...
(* ------------------------------------------------------------------------- *)
  ...
  let vEQ_MP (Sequent(asl1,eq)) (Sequent(asl2,c)) =
    match eq with
      Comb(Comb(Const("=",_),l),r) when alphaorder l c = 0
        -> Sequent(term_union asl1 asl2,r)
    | _ -> failwith "EQ_MP"

  let vDEDUCT_ANTISYM_RULE (Sequent(asl1,c1)) (Sequent(asl2,c2)) =
