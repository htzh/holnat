...
let vFINITE_INDEX_NUMSEG = prove
...
let vFINITE_INDEX_NUMBERS = prove
...
let vFINITE_SUPPORT_DELTA = prove
...
let vITERATE_RELATED_NONEMPTY = prove
...
let vITERATE_ITERATE_PRODUCT = prove
...
let vITERATE_RESTRICT_SET = prove
...
let vITERATE_EQ_GENERAL_INVERSES = prove
...
let vITERATE_UNION_NONZERO = prove
...
let vITERATE_IMAGE_NONZERO = prove
...
let vITERATE_CLAUSES_NUMSEG = prove
...
let vITERATE_CLAUSES_NUMSEG_LT = prove
...
let vITERATE_CLAUSES_NUMSEG_LE = prove
...
let vITERATO_EXPAND_CASES = prove
...
let vITERATO_CLAUSES_GEN = prove
...
let vITERATO_CLAUSES_EXISTS = prove
...
let vITERATO_CLAUSES_NUMSEG_LEFT = prove
...
let vNSUM_GROUP_RELATION = prove
...
let vNSUM_CLOSED_NONEMPTY = prove
...
let vNSUM_RELATED_NONEMPTY = prove
...
let vSUM_CLOSED_NONEMPTY = prove
 ((parse_term "!P f:A->real s.\n        FINITE s /\\ ~(s = {}) /\\\n        (!x y. P x /\\ P y ==> P(x + y)) /\\ (!a. a IN s ==> P(f a))\n        ==> P(sum s f)"),
  vREPEAT vSTRIP_TAC ---->
...
let vSUM_RELATED_NONEMPTY = prove
 ((parse_term "!R (f:A->real) g s.\n        (!m n m' n'. R m n /\\ R m' n' ==> R (m + m') (n + n')) /\\\n        FINITE s /\\ ~(s = {}) /\\ (!x. x IN s ==> R (f x) (g x))\n        ==> R (sum s f) (sum s g)"),
  vREWRITE_TAC[vIMP_CONJ; vRIGHT_FORALL_IMP_THM] ---->
  vGEN_TAC ----> vREPEAT vDISCH_TAC ---->
  vMP_TAC(vISPEC (parse_term "R:real->real->bool")
...
let vREAL_OF_NUM_SUM_GEN = prove
...
let vREAL_OF_NUM_SUM_NUMSEG = prove
...
let vREAL_SUB_POLYFUN_ALT = prove
...
let vREAL_POLYFUN_ROOTBOUND = prove
...
let vREAL_POLYFUN_FINITE_ROOTS = prove
...
let vREAL_POLYFUN_EQ_CONST = prove
...
let vPOLYNOMIAL_FUNCTION_CONST = prove
...
let vPOLYNOMIAL_FUNCTION_ID = prove
...
let vPOLYNOMIAL_FUNCTION_LMUL = prove
...
let vPOLYNOMIAL_FUNCTION_RMUL = prove
...
let vPOLYNOMIAL_FUNCTION_NEG = prove
...
let vPOLYNOMIAL_FUNCTION_SUB = prove
 ((parse_term "!p q. polynomial_function p /\\ polynomial_function q\n         ==> polynomial_function (\\x. p x - q x)"),
  vSIMP_TAC[real_sub; vPOLYNOMIAL_FUNCTION_NEG; vPOLYNOMIAL_FUNCTION_ADD]);;

let vPOLYNOMIAL_FUNCTION_MUL = prove
 ((parse_term "!p q. polynomial_function p /\\ polynomial_function q\n         ==> polynomial_function (\\x. p x * q x)"),
  vREWRITE_TAC[vIMP_CONJ; vRIGHT_FORALL_IMP_THM] ---->
  vGEN_TAC ----> vDISCH_TAC ---->
...
let vPOLYNOMIAL_FUNCTION_SUM = prove
...
let vPOLYNOMIAL_FUNCTION_POW = prove
...
let vPOLYNOMIAL_FUNCTION_INDUCT = prove
...
let vPOLYNOMIAL_FUNCTION_o = prove
...
let vPOLYNOMIAL_FUNCTION_FINITE_ROOTS = prove
